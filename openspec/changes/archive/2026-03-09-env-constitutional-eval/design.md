...
- B) evolve-scorer エージェント流用 → スコアリングの粒度が合わない

**理由**: 既存の evolve-anything パターン（LLM 依存を外部プロセスに閉じ込める）と整合。haiku モデルでコスト最小化。失敗時は Constitutional Score なしで environment fitness を算出可能（graceful degradation）。

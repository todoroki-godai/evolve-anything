...
生成された eval set は `~/.claude/evolve-anything/eval-sets/<skill-name>.json` に保存する（SHALL）。既存ファイルがある場合は上書きする。

#### Scenario: File output path
- **WHEN** `aws-cdk-deploy` の eval set が生成される
- **THEN** `~/.claude/evolve-anything/eval-sets/aws-cdk-deploy.json` に保存される

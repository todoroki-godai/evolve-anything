...
元スキルに YAML frontmatter（`---` で始まるブロック）が存在する場合、パッチ後のコンテンツにも frontmatter が存在しなければならない（MUST）。frontmatter が消失している場合はゲート不合格としなければならない（MUST）。ゲートチェックの実装は `scripts/lib/regression_gate.py` の `check_gates()` を使用しなければならない（MUST）。

...
- `optimize` / `evolve-loop`: `_check_line_limit()` を共通モジュールに移動（機能変更なし）

## Impact

- **変更対象ファイル**: `skills/evolve-fitness/scripts/fitness_evolution.py`, `skills/generate-fitness/templates/fitness-template.py`, `skills/enrich/scripts/enrich.py`, `skills/genetic-prompt-optimizer/scripts/optimize.py`, `skills/evolve-loop-orchestrator/scripts/run-loop.py`

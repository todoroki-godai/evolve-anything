...
### Requirement: /evolve-anything:prune スキルで未使用アーティファクトを淘汰しなければならない（MUST）
...
淘汰対象は .claude/evolve-anything/archive/ に移動しなければならない（MUST）。直接削除を行ってはならない（MUST NOT）。

#### Scenario: アーカイブ
- **WHEN** ユーザーが淘汰候補を承認する
- **THEN** 対象ファイルが .claude/evolve-anything/archive/ に移動される

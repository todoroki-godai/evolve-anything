...
- **WHEN** openspec プラグインのスキルが合計 340 回、evolve-anything プラグインのスキルが 30 回使用されている
- **THEN** `Plugin usage: openspec(340) / evolve-anything(30)` と表示される
...
discover の `detect_behavior_patterns()` はプラグインスキルをメインランキングから除外し、plugin_summary エントリとして末尾に付加する。

...
evolve-anything の observe hooks（PostToolUse, SubagentStop）は Claude Code セッション中のツール呼び出しを JSONL に記録するが、導入前のセッション履歴は対象外。Claude Code は `~/.claude/projects/<encoded-path>/*.jsonl` にセッショントランスクリプトを保存しており、`type: "assistant"` レコード内の `tool_use` ブロックから Skill/Agent 呼び出しを復元できる。
...
- `/evolve-anything:backfill` スキルとして任意のプロジェクトで実行可能にする
...
| 4 | スキルで提供（`/evolve-anything:backfill`）| ユーザーが任意のタイミングで実行可能。evolve の前提条件としても使える |

...
- **audit に Memory Health セクション追加**: `/evolve-anything:audit` 実行時にMEMORYファイルの内容を分析し、陳腐化参照・肥大化警告・改善提案を含む "## Memory Health" セクションをレポートに追加する
- **reflect に Memory Update Candidates 追加**: `/evolve-anything:reflect` 実行時に corrections と既存MEMORYエントリを照合し、既存エントリの更新が必要な候補を `memory_update_candidates` として出力に追加する

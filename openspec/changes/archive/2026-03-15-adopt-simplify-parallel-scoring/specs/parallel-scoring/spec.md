...
### Requirement: evolve-scorer SHALL use 3 parallel sub-agents for scoring
evolve-scorer（`agents/evolve-scorer.md`）はオーケストレーターとして動作し、3つのサブエージェントを Agent ツールで並列起動して採点する。各サブエージェントは1つの評価軸に特化する。

#### Scenario: Normal scoring with 3 parallel sub-agents
- **WHEN** evolve-scorer が対象ファイルの採点を開始する
...
`agents/evolve-scorer.md` の frontmatter `model` は `haiku` とする。

#### Scenario: Orchestrator model
- **WHEN** evolve-scorer エージェントが起動する

...
ユーザーがタスクを切り替えた際に、該当するスキルが存在するにもかかわらず手動で作業してしまう「スキル見落とし」が発生している。現状の evolve-anything はこのパターンを検出できず、ユーザー自身が気づいて初めて correction が発生する。また、correction からルール/メモリを提案する reflect のスコープ判定がキーワードベースのため、global vs project-specific の配置が不正確になるケースがある。

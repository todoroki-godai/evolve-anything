...
`telemetry_query.py` の query_usage() / query_errors() / query_sessions() に `since` / `until` パラメータ（ISO 8601 文字列）を追加する。
...
`telemetry.py` は `--fitness` フラグでは使用しない（既存 fitness インターフェースは stdin にスキル内容を受け取るため、プロジェクトパスが必要な telemetry.py とは互換性がない）。代わりに argparse ベースの CLI を提供する: `python3 telemetry.py <project_dir> [--days N]`。audit 統合のみが公開インターフェースとなる。

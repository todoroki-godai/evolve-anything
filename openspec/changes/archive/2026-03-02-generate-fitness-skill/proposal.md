...
evolve-anything を各プロジェクトにインストールしても、組み込みの評価関数（`default`, `skill_quality`）はドメイン非依存の汎用チェックしかできない。プロジェクト固有の品質基準（例: docs-platform の front matter 必須ルール、ooishi-kun の personality 適合性）を反映した評価関数を手動で書く必要があり、導入障壁が高い。

## What Changes

- **新スキル `/generate-fitness`** を evolve-anything plugin に追加

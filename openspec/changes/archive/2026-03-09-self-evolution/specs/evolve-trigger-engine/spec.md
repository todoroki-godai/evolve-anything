...
- **THEN** `TriggerResult(triggered=True, reason="self_evolution", action="/evolve-anything:evolve")` を返す
...
- **THEN** `TriggerResult(triggered=True, reason="approval_rate_decline", action="/evolve-anything:evolve")` を返す

...
さらに、evolve-anything プラグイン自身のスキルに frontmatter がなく、Claude が自動起動判定できない。

## What Changes

- `find_artifacts()` にスキルの出自分類（custom / plugin / global）を追加
- `detect_zero_invocations()` でプラグイン由来スキルを淘汰対象外にし、レポートのみ出力
- evolve-anything プラグインの全 SKILL.md に YAML frontmatter を追加（description / disable-model-invocation）
...
- `frontmatter-standardization`: evolve-anything プラグインの全 SKILL.md に YAML frontmatter を追加し、発見性を向上させる

...
evolve-anything プラグインの全スキル SKILL.md ファイルに YAML frontmatter を追加しなければならない (MUST)。
...
- **THEN** Claude が自動起動せず、ユーザーが `/evolve-anything:prune` で明示的に呼び出さなければならない (MUST)
...
`genetic-prompt-optimizer`, `evolve-loop-orchestrator`, `generate-fitness` は既に適切な frontmatter を持つため、既存の frontmatter を維持しなければならない (MUST)。
...
`openspec-apply-change`, `openspec-explore`, `openspec-verify-change`, `openspec-archive-change`, `openspec-propose` は openspec プラグインがインストールしたスキルであり、evolve-anything の管理対象外のため変更してはならない (MUST)。
commands 削除後の発見性は、既存の openspec-* description と `/opsx:*` エイリアスで維持される。

#### Scenario: インストールスキルの保護
- **WHEN** evolve-anything の evolve-skill-classification 変更を適用する

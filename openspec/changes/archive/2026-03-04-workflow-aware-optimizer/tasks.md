...
- [x] 1.1 `scripts/rl/workflow_analysis.py` を作成。workflows.jsonl を読み取りスキル別統計 JSON を `~/.claude/evolve-anything/workflow_stats.json` に出力
...
## 3. evolve-scorer ワークフロー品質軸

- [x] 3.1 agents/evolve-scorer.md にワークフロー効率性の補助評価軸を追加
- [x] 3.2 ワークフロー統計 JSON 参照の指示を追加（存在時のみ使用、なければスキップ）

## 4. generate-fitness ワークフロー統計 + `--ask` 対応

- [x] 4.1 analyze_project.py にワークフロー統計 JSON（`~/.claude/evolve-anything/workflow_stats.json`）のマージロジック追加

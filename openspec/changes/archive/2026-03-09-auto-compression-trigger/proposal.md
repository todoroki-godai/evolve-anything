...
- bloat 種別ごとに適切なアクションを提案:
  - MEMORY.md 超過 → `/evolve-anything:evolve`（分割提案）
  - rules 総数超過 → `/evolve-anything:evolve`（統合・prune 提案）
  - skills 総数超過 → `/evolve-anything:evolve`（archive・prune 提案）
  - CLAUDE.md 超過 → `/evolve-anything:evolve`（分割提案）

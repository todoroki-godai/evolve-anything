...
evolve-anything は Claude Code Plugin として 13 スキル + 7 フック + 1 エージェント（evolve-scorer）で構成される。Claude Code v2.1.x で追加された plugin/skill 向け新機能が未適用のまま残っており、コンテキスト効率・コスト・安全性に改善余地がある。
...
- evolve-scorer は既に `model: sonnet`, `memory: project` を設定済み
...
**除外**: reflect（対話的レビューが必要で会話コンテキストに依存）、optimize/evolve-loop（corrections コンテキストが必要）、version/feedback/update（軽量で fork 不要）。
...
| 用途 | モデル | 理由 |
|------|--------|------|
| evolve-scorer（採点） | sonnet（現状維持） | コスト効率 + 十分な品質 |
...
**代替案**: auto-memory を完全無効化 → ユーザー体験が悪化。evolve-anything 側の memory 書き込みを廃止 → reflect の価値が減少。semantic embedding → 外部依存が増加、現時点では Jaccard で十分。

...
`hooks/common.py` の `DATA_DIR` は `${CLAUDE_PLUGIN_DATA}` 環境変数が設定されていればそれを使用し、未設定時は `~/.claude/evolve-anything/` にフォールバックする（SHALL）。
...
- **THEN** `DATA_DIR` は `Path.home() / ".claude" / "evolve-anything"` となる

#### Scenario: CLAUDE_PLUGIN_DATA が空文字列の場合
- **WHEN** 環境変数 `CLAUDE_PLUGIN_DATA` が空文字列に設定されている
- **THEN** `DATA_DIR` は従来の `~/.claude/evolve-anything/` にフォールバックする

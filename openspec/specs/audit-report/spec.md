...
- **WHEN** `/evolve-anything:audit --coherence-score` を実行し、coherence スコアが overall=0.85, coverage=1.0, consistency=0.7, completeness=0.9, efficiency=0.8 の場合
...
- **WHEN** `/evolve-anything:audit` を実行する（`--coherence-score` なし）
...
- **THEN** 該当行に "DEGRADED" マーカーを表示し、`/evolve-anything:evolve` での self-evolution を推奨する

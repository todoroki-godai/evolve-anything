...
| Blind Curator（judge false-pass） | LLM judge の false-pass（失敗を合格と誤判定）がスキル退役（negative_transfer rollback）を無音で無効化するサイレント故障。既知の欠陥 fixture を judge の実プロンプトに流す欠陥注入監査（`judge_audit`）で事前計測する | #188, arXiv 2607.07436 |
...
| evolve-proposals（提案バッチレポート） | fleet queue の待ち PJ に `run_evolve(dry_run=True)` の提案だけをバッチ生成し束ねた集約レポート `DATA_DIR/evolve-proposals-<date>.md`+`.json`（`bin/evolve-fleet propose`）。evolve-queue.json と同じ read 専用派生物（store_registry 非登録）。適用はしない（提案の生成・集約のみ、Phase 3 で人間ゲート適用） | #81, daily-evolve Epic #78 |

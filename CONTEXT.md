...
| classification（store 4分類） | store_registry のストア宣言フィールド。status（write barrier の生死）とは独立の軸で store の役割を4分類する: `raw_event`=一次イベント SoR / `workflow_state`=判断・処理進捗の状態 / `derived_cache`=派生・集計キャッシュ / `dead`=削除候補。classification=dead は通常 status=dead を伴うが、write barrier 経由の live なオプトイン CLI 機能等では status 降格を保留する例外がある（`declarations_by_classification()`） | #379 |
...
| icebox | 再開条件付き凍結 issue。closed + label `icebox` で open バックログを汚さず評価内容を保持（tech-eval 3問ゲートの行き先）。daily runner が棚卸し集計し最古N日超（既定30日・userConfig `icebox_review_threshold_days` で調整可）を SessionStart で1行通知（`icebox_notice`） | #194 |
| icebox 3レーン分類 | icebox の再開条件を実ストアと決定論突合し「成立」（レーン1）/「観測器不在」（レーン2）/「失効候補」（レーン3）へ判定する `icebox_reconcile` の分類（優先順位 met > observer_missing > archive_candidate）。成立分のみ SessionStart が名指し通知、観測器不在・失効候補は audit advisory | #352 |
| reopen-when | icebox issue 本文 `## 再開条件` 配下の fenced YAML ブロック契約（`source`/`metric`/`op`/`threshold`）。`icebox_reconcile` が weak_signals/subagent_traces/token_usage 等の evaluator と決定論突合し成立判定する入力 | #352 |
...
| write barrier | 全ストア書込の単一ゲート `rl_common.store_write(store_name, record)`。canonical `DATA_DIR/<name>` を内部解決し（呼び出し側は保存先指定不可）、store_registry の active 登録を runtime guard（既定 reject・未登録/非active は `StoreWriteError`）で照合。kind=json ストア（単一 JSON オブジェクト丸ごと上書き・evolve-state.json 等）への jsonl append も reject（単一 JSON へ行追記するとファイルが壊れるため・#399）。read（union 寛容）と write（canonical 厳格）を分離し共有は store_registry のみ。緊急避難は env `EVOLVE_WRITE_GUARD=warn` | ADR-049, #55, #399 |
...
| evolve-tier | モデルティア（HEAD/HARD/NORMAL/MECH/REVIEW ↔ model/effort）正典の一元管理 CLI（`bin/evolve-tier`）。`set` で正典更新、`sync [--apply]` で agent frontmatter/settings.json/routing rule マーカーへ反映（既定 dry-run・targets 明示列挙のみ・自動検出なし）、`drift` で正典に無いモデルエイリアスの散文残存を検出。model-tiers.json は DATA_DIR ストアでなく `~/.claude/` 配下のユーザー config（store_registry 非登録） | #193 |
| model-tiers.json | evolve-tier の正典 config（`~/.claude/model-tiers.json`）。tiers（model/effort/description）+ targets（sync 対象の明示列挙: agents/settings/routing_rules）+ advisory_scan（drift 検出対象ディレクトリ）を保持。call-time 解決（`tier_policy.tiers_config_path()`）でモジュールレベルにキャッシュしない | #193 |
| TRUSTMEM | 記憶更新の遷移検証。auto-memory が同名の既存 memory エントリを更新する書込に対し、coverage（重要行の大量欠落）/ preservation（`metadata.type` の矛盾上書き）/ fidelity（冒頭行の極性反転疑い）を difflib ベースで決定論検証するゲート。同名エントリが無い書込は検証対象外（block しない）。memory_guard.py に #108（汚染検出）の拡張として追加 | #93, arXiv 2606.25161 |
| worker-takeoff（completed≠完遂） | subagent が harness に completed 扱いされたのに、最終出力が完了報告でなく中間ナレーションのまま終わっている乖離。`detect_takeoff_divergence` が subagents.jsonl の `last_assistant_message` から決定論2シグナル AND（完了署名の欠如×前向きナレーション終端）で検出（`worker_takeoff`）。500字打ち切り到達時は判定不能 | #161, arXiv 2607.02507 |
| ゾンビ宣言 | SKILL.md 散文が「`func(args)` を実行する」と宣言している callable が、定義モジュール自身と `tests/` 以外どこからも呼ばれていない（到達不能）状態。#170 の `intention_check` が原型。`skill_declaration_reachability.py`（`skill_reachability`）が AST 静的解析で決定論検出し audit advisory + dogfood gate light 非ブロッキング警告に surface | #191, #170 |
| evolve-proposals（提案バッチレポート） | fleet queue の待ち PJ に `run_evolve(dry_run=True)` の提案だけをバッチ生成し束ねた集約レポート `DATA_DIR/evolve-proposals-<date>.md`+`.json`（`bin/evolve-fleet propose`）。evolve-queue.json と同じ read 専用派生物（`.json` は store_registry に derived_cache 登録済み・`.md` は宣言対象外・#399 codex round1 是正）。適用はしない（提案の生成・集約のみ、Phase 3 で人間ゲート適用） | #81, daily-evolve Epic #78 |
| evolve-apply worktree | evolve-proposals（#81）を承認後に人間が対話 evolve で適用する隔離作業場。`evolve-fleet pr-start` が repo 外の configurable root に `<executor>/<issue>-<slug>-<suffix>` worktree と `<executor>/<issue>-<slug>` branch を作成し、適用そのものは人間が中で `/evolve-anything:evolve` を回す（自動化しない）。`evolve-fleet pr-finish` は必須path allowlistだけをcommitし、pushアカウント検証→push→`gh pr create`までを行う。マージは常に人間 | #82, #268, daily-evolve Epic #78 |
| evaluation provenance | 評価スコアに紐づく実行条件（harness）の記録契約。`scripts/lib/evaluation_provenance.py` の envelope（`schema_version` / `evaluation_kind` / `producer` / `runtime` / `plugin` / `judge` / `config` / `inputs` / `recorded_at`）を、producer が組み立て → 永続化境界で `finalize_provenance` が共通項目を補完 → store はそのまま append、の責務分離で残す。**不明値は推測せず None**、決定論評価に `judge` キーは付けない（渡すと `ValueError`＝非該当と観測不能の区別を守る）。組立に失敗しても provenance キーごと落とさず `evaluation_kind=unknown` で残す（無記録だと契約 drift が全緑のままデータ欠損になる）。過去分は遡及不能なので記録だけ先に始める（条件別の比較・交絡補正は #240 で凍結中） | #309 |
| mixed_provenance | constitutional の集約スコアで、layer cache が別時点・別モデルで生成されたものを含む状態。集約を単一条件へ潰さず `judge_models`（集合）+ `plugin_versions`（layer 由来の版集合）+ `harness_variants`（model / effort / tool policy / 版 / runtime の正規形集合）+ `mixed_provenance`（真偽）で表現する。混在判定は model 単独でなく harness tuple 全体（同じモデルでも effort や版が違えば別条件）。envelope の `plugin.version` は「集約した時点」の版なので layer 由来の版とは別に持つ。provenance を持たない旧 layer が 1 つでもあれば「揃っていない」扱い | #309 |
| 新設凍結 | 縮小方針確定（#379）後、縮小完了まで新 store / observability section / advisory proposal adapter / weak_signal channel の**追加**を停止する契約（削除は常に許容）。単一ソース `scripts/lib/shrink_freeze.py` が実装時点のレジストリを frozenset で凍結し、`assert_no_new_keys` が拡張方向のみ reject する | #379 |
| 表示淘汰 | #379 Step 2。人間の行動に繋がった実証のない observability section 33 件を audit の**表示からのみ**外す（コードは削除しない）。単一ソース `shrink_freeze.CULLED_OBSERVABILITY_SECTIONS`。`collect_observability()` が該当 key の builder 呼び出しを skip し、淘汰した事実は `display_cull` の 1 行 meta で必ず surface する。`EVOLVE_SHOW_CULLED=1` で一時的に全表示へ戻せる | #379 |
| effective view（判断母集団） | optimize_history の「生ログ」に対し、revert 済み accept を畳んだ読み取りビュー。`fold_effective` が唯一の畳み込みロジックで、業務 reader は `load_effective_history` を使う。revert された entry は**除外**し（`reverted` フラグを立てない＝各 reader が個別に除外を書く余地を残さない）、revert イベント自体も含めない。revert の事実が要る reader は `load_revert_events` を使う | #402, ADR-053 |
| raw history allowlist gate | 生ログ reader を閉じた許可リストに固定する AST 静的 gate（`raw_history_gate`）。未許可の新規呼出しも、実体が消えた allowlist entry（`stale_allowlist`）も fail。判定は bare 名でなく import 元・関数/callsite 単位。許可の単一ソースは production 定数 `PRODUCTION_ALLOWLIST`（現在3件） | #402, ADR-053 |

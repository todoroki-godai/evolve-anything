...
| icebox | 再開条件付き凍結 issue。closed + label `icebox` で open バックログを汚さず評価内容を保持（tech-eval 3問ゲートの行き先）。daily runner が棚卸し集計し最古90日超を SessionStart で1行通知（`icebox_notice`） | #194 |

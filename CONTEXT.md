...
| 表示淘汰 | #379 Step 2。人間の行動に繋がった実証のない observability section 35 件を audit の**表示からのみ**外す（コードは削除しない）。単一ソース `shrink_freeze.CULLED_OBSERVABILITY_SECTIONS`。`collect_observability()` が該当 key の builder 呼び出しを skip し、淘汰した事実は `display_cull` の 1 行 meta で必ず surface する。`EVOLVE_SHOW_CULLED=1` で一時的に全表示へ戻せる | #379 |

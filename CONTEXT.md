...
| evolve-apply worktree | evolve-proposals（#81）を承認後に人間が対話 evolve で適用する隔離作業場。`evolve-fleet pr-start` が対象 PJ に `.claude/worktrees/evolve-apply-<YYYYMMDD>` を `-b evolve/<YYYYMMDD>-proposals` で作成し、適用そのものは人間が中で `/evolve-anything:evolve` を回す（自動化しない）。`evolve-fleet pr-finish` が commit→push アカウント検証→push→`gh pr create` までを自動化し、マージは常に人間 | #82, daily-evolve Epic #78 |

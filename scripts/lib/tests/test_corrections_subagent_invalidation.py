...
（source_path に /subagents/ を含む）を corrections レコードに channel 不問で適用し、
既存の `invalidated` フラグ（安全弁③・provenance_weight.is_human_correction / growth_report
が既に除外条件として読む）で論理無効化する。物理削除はしない。

**channel は問わない**（2026-08-13 頭の判断で拡大）。ADR の「残り49件は TTL 自然失効に任せる」の
"49件"は未昇格 weak_signal を指すのであって、既に corrections まで到達した rephrase channel の
6件は llm_judge の2件と同じ状態＝同一の後始末対象（実データでは計8件）。

dry-run 既定（安全側）。すべて LLM-free・決定論。
"""
import json
import sys
...
SUBAGENT_REPHRASE = {
    # channel 不問で対象（2026-08-13 判断拡大）: rephrase 由来でも source_path に /subagents/
    "source": "reflect_confirmed",
    "correction_type": "semantic_idiom",
    "weak_signal_key": "1c22c8ecabb3bf7a",
...
NORMAL_REPHRASE = {
    # rephrase channel でも source_path が subagents でなければ対象外
    "source": "reflect_confirmed",
    "correction_type": "semantic_idiom",
...
def test_missing_corrections_file_returns_empty_report(tmp_path):
    report = csi.invalidate_subagent_contaminated_corrections(tmp_path / "nope.jsonl")
...
def test_dry_run_default_reports_candidates_without_writing(tmp_path):
    ...
    _write_jsonl(corr, [
        SUBAGENT_LLM_JUDGE, NORMAL_LLM_JUDGE, SUBAGENT_REPHRASE, NORMAL_REPHRASE, ALREADY_INVALIDATED,
    ])

    report = csi.invalidate_subagent_contaminated_corrections(corr)  # dry_run 既定

    assert report["dry_run"] is True
    # channel 不問: llm_judge の subagent 汚染 + rephrase の subagent 汚染の両方が候補に入る
    assert set(report["candidates"]) == {"836826fb11c47e48", "1c22c8ecabb3bf7a"}
    assert report["invalidated"] == 0
...
def test_apply_invalidates_matching_records_only(tmp_path):
    ...
    report = csi.invalidate_subagent_contaminated_corrections(corr, dry_run=False)

    assert report["dry_run"] is False
    assert report["invalidated"] == 2
    records = _read_jsonl(corr)
    by_key = {r["weak_signal_key"]: r for r in records}
    for key in ("836826fb11c47e48", "1c22c8ecabb3bf7a"):
        target = by_key[key]
    ...
    # source_path が subagents でない通常レコード・既invalidated は無傷
...
def test_apply_is_idempotent_on_second_run(tmp_path):
    corr = tmp_path / "corrections.jsonl"
    _write_jsonl(corr, [SUBAGENT_LLM_JUDGE, SUBAGENT_REPHRASE])
    csi.invalidate_subagent_contaminated_corrections(corr, dry_run=False)

    report2 = csi.invalidate_subagent_contaminated_corrections(corr, dry_run=False)

    assert report2["candidates"] == []  # 既に invalidated=True は候補に出ない
    assert report2["invalidated"] == 0
    records = _read_jsonl(corr)
    assert len(records) == 2  # 二重書きされない
...
def test_invalidated_record_excluded_from_count_human_corrections(tmp_path):
    ...
    corr = tmp_path / "corrections.jsonl"
    _write_jsonl(corr, [SUBAGENT_LLM_JUDGE, SUBAGENT_REPHRASE, NORMAL_LLM_JUDGE])
...
def test_main_dry_run_default_does_not_write(tmp_path, monkeypatch, capsys):
    ...
    assert "候補: 2 件" in out
    assert "無効化: 0 件" in out
    records = _read_jsonl(corr)
    assert all(r["invalidated"] is False for r in records)
...
def test_main_apply_writes(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "無効化: 2 件" in out
    records = _read_jsonl(corr)
    assert all(r["invalidated"] is True for r in records)

...
def _seed(db: Path) -> None:
    rows = [
        Utterance("/p/a.jsonl", 1, "rl-anything", "s1", "2026-05-01T00:00:00Z",
                  "古い対話", "h1", None, "dialogue", 1),
        Utterance("/p/a.jsonl", 2, "rl-anything", "s1", "2026-06-09T00:00:00Z",
                  "新しい対話", "h2", None, "dialogue", 1),
        Utterance("/p/a.jsonl", 3, "rl-anything", "s1", "2026-06-09T00:01:00Z",
...
def test_opt_in_long_paste(tmp_path: Path) -> None:
    ...
    rows = uquery.query_utterances(
        "rl-anything", source_kinds=("dialogue", "long_paste"), db_path=db
    )
...
def test_since_returns_old_and_new(tmp_path: Path) -> None:
    ...
    db = tmp_path / "u.db"
    _seed(db)
    all_rows = uquery.query_utterances("rl-anything", db_path=db)
    assert any(r["text"] == "古い対話" for r in all_rows)
    # since で絞ると古いものは落ちる
    recent = uquery.query_utterances("rl-anything", since="2026-06-01T00:00:00Z", db_path=db)
...
def test_keyword_filter(tmp_path: Path) -> None:
    db = tmp_path / "u.db"
    _seed(db)
    rows = uquery.query_utterances("rl-anything", keyword="新しい", db_path=db)
    assert len(rows) == 1
...
def test_all_projects_crosses_pj(tmp_path: Path) -> None:
    ...
    assert "rl-anything" in slugs and "otherpj" in slugs


def test_query_missing_db_returns_empty(tmp_path: Path) -> None:
    rows = uquery.query_utterances("rl-anything", db_path=tmp_path / "nope.db")
    assert rows == []

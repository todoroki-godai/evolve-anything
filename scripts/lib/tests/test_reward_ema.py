...
HOME 隔離は root conftest.py の autouse（#119・旧 #457）が全テストへ効かせる。各テストは
``monkeypatch.setattr(reward_ema, "DATA_DIR", tmp_path)``（predictive_validity と同型）で
read 先を tmp に向ける。write 経路（store_write）は ``rl_common.DATA_DIR`` を見るので、
persist テストではそちらも tmp に向ける。
"""

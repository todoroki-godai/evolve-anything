...
evolve-anything 本体 skills / インストール済みプラグイン skills）にも実在しないケース。
...
def _repo_self_skill_names() -> frozenset:
    """evolve-anything 本体リポジトリの skills/ 直下の skill 名集合。

    `/evolve-anything:implement` 等のプラグイン参照は通常 _plugin_skill_names でも拾えるが、
...
def build_live_skill_registry() -> frozenset:
    ...
    情報源を併合する:
      ...
      - evolve-anything 本体 repo の skills/（開発中フォールバック）

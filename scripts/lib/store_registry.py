...
_DECLARATIONS: List[StoreDeclaration] = [
    ...
    StoreDeclaration(
        ...
        "+ proposals digest（#409・scripts/lib/daily/proposal_digest.py）と weekly_board（#401）を埋め込んで "
        "保存。毎朝1回・日次上書き）。",
        writer_locus="batch",
        ...
        "判定）/ scripts/lib/session_notify/weekly_board_notice.py（weekly_board の SessionStart 提示）/ "
        "scripts/lib/fleet/cli_propose.py（--live 未指定時の propose 入力）。",
        retention="permanent",
        classification="derived_cache",
        note="#379 Step 4 PR E round2（#399 codex Must 2）: `fleet queue --json` 出力の "
        "日次スナップショット（毎朝上書き・SoR は fleet queue の元データ）。#409 で "
        "proposals フィールド（改善案 digest）、#401 で weekly_board（週次戦果要約）を追加。",
    ),
    StoreDeclaration(

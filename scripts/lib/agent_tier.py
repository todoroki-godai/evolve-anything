...
正典（HEAD=sonnet/max, HARD=sonnet/xhigh, NORMAL=sonnet/medium, MECH=haiku/effort無し,
REVIEW=fable/high）を単一ソースとして写す。2026-07-10: opus 4.8 使用セッションで
自己汚染ハルシネーション指紋が集中する実測を受け、HEAD/HARD の model を opus から
撤去（tier 名は維持）。HARD は sonnet・effort xhigh。HEAD は fable と sonnet を
往来した末 settings.json 実態（sonnet・max）に確定。**この往来自体が「モデル変更を
一元管理する仕組みが無い」ことの実例（#未起票、次アクションで解消予定）** —
実装されたらこのモジュールが tier→model/effort の唯一の書込み先になる想定。

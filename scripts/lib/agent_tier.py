...
ポリシー（HEAD/HARD/NORMAL/MECH/REVIEW ↔ model/effort）の正典は `tier_policy.py`
（`~/.claude/model-tiers.json` の外部 config、#193）へ移管済み。2026-07-10: opus 4.8
使用セッションで自己汚染ハルシネーション指紋が集中する実測を受け、HEAD/HARD の
model を opus から撤去（tier 名は維持）。HARD は sonnet・effort xhigh。HEAD は
fable と sonnet を往来した末 settings.json 実態（sonnet・max）に確定。**この往来
自体が「モデル変更を一元管理する仕組みが無い」ことの実例だった（#193 で解消）** —
`tier_policy` が tier→model/effort の正典読み書きの唯一の場所になった。
...
# ティア↔model/effort の正典は tier_policy へ移管（#193）。ここに残す TIER_POLICY は
# gate 互換形（description を落とした {model, effort}）の **静的デフォルト**
# スナップショット（`tier_policy.DEFAULT_TIER_POLICY` 由来）で、後方互換のためだけに
# 残す（実importer は Grep で未検出・将来の参照用）。実際の適合判定
# （check_agent_tier）は呼び出しごとに `tier_policy.load_tier_policy(strict=False)`
# を呼び、ユーザー config（~/.claude/model-tiers.json）があればそれを期待値として
# 使う（call-time 解決・モジュールレベルにキャッシュしない）。
TIER_POLICY: Dict[str, Dict[str, Optional[str]]] = {
    tier: {"model": policy["model"], "effort": policy["effort"]}
...
def check_agent_tier(frontmatter: Mapping[str, Any]) -> List[Dict[str, Any]]:
    ...
    # 期待値は call-time で tier_policy から解決する（#193）。ユーザー config
    # （~/.claude/model-tiers.json）があればそれを期待値として使う。モジュール
    # レベルにキャッシュしない（config の実行時変更に追従するため）。
    policy_map = tier_policy.load_tier_policy(strict=False)

    # tier が無い / 未知 → tier 依存チェックは判定不能ゆえ missing_tier のみ返す。
    if tier is None or tier not in policy_map:
        detail = (

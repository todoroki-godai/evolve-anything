...
1b. 最大長 ceiling: 正規化後 ``MAX_IDIOM_CHARS`` 文字超を弾く（メッセージ全文＝過特化で
   二度と同テキスト再発しないため standing auto-promote rule にしない・#22）。
...
# ── ゲート①b: 最大長 ceiling（過特化＝メッセージ全文を弾く・#22）──────────────
# floor が「過汎用な極短 idiom」を弾くのに対し、ceiling は逆方向の FP を弾く。
# 冗長な（複数行・自然文の）暗黙修正シグナルでは confirmable_idiom にメッセージ全文が
# そのまま入る。idiom_autopromote（#463）は confirmed idiom と**同テキストの**再発を機械
# 昇格するため、全文一致 idiom は実運用で二度と再発せず standing auto-promote rule にしても
# 将来価値ゼロ・辞書にノイズが溜まるだけ。#527 の FP guard は「過汎用」側しか見ておらず
# 「過特化（全文）」側を素通ししていた。正規化後 ``MAX_IDIOM_CHARS`` 文字超を弾く
# （今回限りの昇格は別経路で許可・standing rule にしないだけ）。
# 80 文字: median 10 文字の生発話断片（bootstrap_backlog 知見）の十分上、かつ「一文の短い
# 修正指示」（"四国めたんじゃなくてつむぎにして" 等）は通る現実的上限。複数行の自然文
# （フッター + 余白 + 色 + 見出し… のような複合指示）はここを超える。
MAX_IDIOM_CHARS = 80
...
def eligible_reason(text: Optional[str]) -> Optional[str]:
    ...
    順序: too_short → too_long → context_token → stopword（弾く理由の代表値を 1 つ返す）。
    """
    norm = _normalize(text)
    if len(norm) < MIN_IDIOM_CHARS:
        return "too_short"
    if len(norm) > MAX_IDIOM_CHARS:
        return "too_long"
...
def idiom_eligible(text: Optional[str]) -> bool:
    ...
    ゲート（floor / ceiling / stopword / context token）を全通過したら True。
    """

...
def backfill(data_dir: Path, *, apply: bool = False) -> Dict[str, Any]:
    """3ストアの project / project_path を worktree 安全 slug に遡及正規化する。
    ...
    corrections = _backfill_jsonl(data_dir / "corrections.jsonl", "project_path", apply=apply)
    subagents = _backfill_jsonl(data_dir / "subagents.jsonl", "project", apply=apply)
    sessions_db = _backfill_sessions_db(data_dir / "sessions.db", apply=apply)
    return {
        "applied": apply,
...
def format_summary(summary: Dict[str, Any]) -> str:
    ...
    total_norm = sum(
        ...
        for k in ("corrections", "subagents", "sessions_db")
    )

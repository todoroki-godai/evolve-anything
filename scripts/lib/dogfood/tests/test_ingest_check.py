...
高速検証する。実 PJ E2E は bin/rl-dogfood-gate --layer all の実機1周で別途確認する。
"""
...
def _user_line(text, ts, sid, uuid, cwd="/Users/x/tools/rl-anything"):
    return json.dumps(
        {
...
def test_find_real_pj_returns_none_when_absent(monkeypatch, tmp_path):
    # 隔離 HOME 配下に rl-anything PJ が無い → None

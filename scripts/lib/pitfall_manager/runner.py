"""Pitfall hygiene オーケストレータ。

自己進化済みスキル全体を走査して卒業判定 / 統合済み判定 / Stale Knowledge ガード /
Pre-flight スクリプト化候補 / Active 上限 / TTL アーカイブ / 行数ガード /
横断分析 / 合理化防止テーブルを集約する。
"""
import sys
...
from .detection import detect_archive_candidates, detect_integration
from .parser import get_cold_tier, parse_pitfalls
from .preflight import _compute_line_guard, suggest_preflight_script
from .rationalization import generate_rationalization_table

# Cold 層自動アーカイブ定数 (__init__.py からも参照可能なよう SoT)

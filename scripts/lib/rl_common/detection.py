...
_MACHINERY_MARKERS = (
    ...
    # ADR-054 A2（#379/#454）: weak_signals._DISPATCH_MARKERS（rephrase 検出の委譲プロンプト
...
# ADR-054 A2（#379/#454）: 並列 agent 派遣テンプレの広い語彙判定。is_machinery_prompt に
...
def is_dispatch_template_marker(content: str) -> bool:
    """並列 agent 派遣テンプレの広い語彙判定（rephrase の近接高類似ペア専用、#454）。

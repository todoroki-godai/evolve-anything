...
``rl_common.DATA_DIR`` が既定 fallback ``~/.claude/rl-anything`` に解決される。
...
解決順（決定論）:
  ...
  3. base が既定 fallback かつ env も無いとき install レイアウト
     ``~/.claude/plugins/data/<*rl-anything*>`` を探索（tool/skill 実行で
     hook の書いた dir を回収）
...
_REAL_DEFAULT_FALLBACK = Path.home() / ".claude" / "rl-anything"
...
def _probe_install_layout(base: Path) -> Optional[Path]:
    """``base`` 配下から rl-anything の plugin-data dir を決定論で 1 つ選ぶ。
    ...
    if len(candidates) > 1:
        print(
            "[rl-anything] multiple plugin-data dirs matched "

...
CANONICAL: List[Key] = [
    ...
    Key("phases.remediation.proposable_global_suppressed", int, note="件数（#103）。情報レーン proposable_global の dismiss 済み（TTL 内）を surface から畳んだ数"),
    Key("phases.remediation.rule_violation_suppressed", int, note="件数（#103）。情報レーン rule_violation_observed の dismiss 済み（TTL 内・violated_command 単位）を surface から畳んだ数"),
    Key("phases.remediation.auto_rejected_by_reconcile", int, note="件数（#494）。連続再出で自動却下した提案数（SKILL.md record_rejection の決定論 fallback・#103 以降は情報レーンの自動畳み込みも含む）"),

...
try:
    from episodic_store import HAS_DUCKDB, insert_event, query_relevant
    from similarity import tokenize
except ImportError:
    # フォールバック: episodic 機能なしで動作継続
    HAS_DUCKDB = False  # type: ignore[assignment]

    def insert_event(*_, **__) -> None:  # type: ignore[misc]
        pass
...
def promote_to_episodic(correction: dict[str, Any]) -> None:
    """適用済み correction を episodic 層に昇格する。

    reflect.py で ユーザーが correction を approve したタイミングで呼び出す。
    DuckDB 未インストール時・エラー時は silent skip。

...
``episodic_events``（episodic_store.py）は #173 まで ``known_store_specs()`` に
未登録だったため、migrate-data の CTAS rebuild で PK が落ちても復元されず
``INSERT OR IGNORE`` が BinderException で writer 全死していた（episodic.db が
全履歴 0 行のまま停止）。#173 で登録し、以後の migration rebuild から保護する。
...
def known_store_specs() -> "list[Tuple[str, str, list, bool]]":
    ...
    try:
        ...
        pass
    try:
        import episodic_store as _epi  # type: ignore

        specs.append(("episodic_events", _epi._SCHEMA_SQL, [("id",)], False))
    except Exception:
        pass
    return specs

...
def build_orphan_store_section(project_dir: Path) -> Optional[List[str]]:
    ...
    def compute(_proj: Path):
        try:
            ...
            return None
        # 検査対象はプラグインソース（hooks/ + scripts/ + skills/）で project_dir 非依存。
        return orphan_store.detect_orphan_stores()
    ...
    return build_advisory_section(
        project_dir,
        title="Orphan Stores (writer あり / reader なし)",
        compute=compute,
        applicable=lambda report: bool(report.reader_count),
        render=render,
    )

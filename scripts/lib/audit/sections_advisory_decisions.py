...
def _render(data: Dict[str, Any]) -> List[str]:
    ...
    lines = [
        f"ℹ advisory 提案の判断を {data['total']} 件記録（detector 別）",
        "",
        "| detector | accept | reject | 採用率 |",
        "|---|---:|---:|---:|",
    ]
    for detector_id in sorted(summary):
        counts = summary[detector_id]
        accept, reject = counts.get("accept", 0), counts.get("reject", 0)
        total = accept + reject
        rate = f"{accept / total:.0%}" if total else "-"
        lines.append(f"| {detector_id} | {accept} | {reject} | {rate} |")
...
def build_advisory_decisions_section(project_dir: Path) -> Optional[List[str]]:
    """detector 別の advisory 提案 accept/reject を audit に surface する。
    ...
    - 記録あり → detector 別テーブル + accept 0 件 detector の ⚠
    """

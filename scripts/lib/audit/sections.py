"""レポートセクション生成（Constitutional Score / Token Consumption / Test Guard / LSP / Corrections）。

audit パッケージから切り出された Sections モジュール。generate_report が呼ぶ
report-format 系のセクション生成関数を集約する。
...
- build_corrections_insights_section: 繰り返し失敗パターン TOP-N
- build_lsp_suggestion_section: LSP未設定PJへの導入提案

環境健全性 observability advisory 系（pitfalls / belief_blocks / calibration_drift /
negative_transfer / glossary_drift）は #122 Phase 4 で sections_meta.py へ物理移設した。
後方互換のため本モジュール末尾で再エクスポートする。
"""
import json
...
# ── Observability advisory builders（#122 Phase 4 で sections_meta.py へ物理移設）──
# pitfalls / belief_blocks / calibration_drift / negative_transfer / glossary_drift の
# 各 builder と private helper を sibling へ移した。従来の import パス（audit.sections /
# observability.py / 各 per-section テスト）を跨がせないため同一オブジェクトを再エクスポート
# する（audit.py 2046→178 split と同じ後方互換方針）。
from .sections_meta import (  # noqa: F401,E402
    _load_count_entries,

...
def _data_dir() -> Path:
    """DATA_DIR を call-time に解決する（#420 整合・store_write と同じ正パターン）。

    旧実装は import 時に ``_DATA_DIR_VAL = _common.DATA_DIR`` でコピー固定し、conftest
    の ``_rebase_module_data_dirs``（#420）の機械 rebase に依存してテスト汚染を免れて
    いた（モジュール単体では env / ``mock.patch.object(rl_common, "DATA_DIR", ...)`` に
    追従できず、許可リスト漏れ時に実 ~/.claude/evolve-anything/ を汚染した — 2026-06-10
    の test_* 56 件が証拠）。本関数は call-time に ``rl_common.DATA_DIR``（patch 経路の
    SoT）を参照し、conftest 防御に非依存でモジュール単体で隔離に追従する。
    """
    try:
        import rl_common

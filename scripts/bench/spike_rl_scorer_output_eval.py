#!/usr/bin/env python3
"""Week 1 スパイク: evolve-scorer の出力評価への転用可否を検証する。

【スパイクの問い】
evolve-scorer はスキル定義（SKILL.md）の品質評価用だが、
LLM が生成した考察系スキル（evolve/reflect/optimize/audit）の**出力テキスト**を
同じ3軸（技術/ドメイン/構造）で評価できるか？

【検証方法】
1. evolve スキルの典型的な出力をサンプルとして用意（モック）
2. evolve-scorer の各軸プロンプトを出力評価用に微調整
...
reflect 完了後も `/evolve-anything:spec-keeper` を提案する（実行確認必須）
```
...
# ──────────────────────────────────────────────
# evolve-scorer の軸プロンプト（出力評価用に調整）
# ──────────────────────────────────────────────
...
DOMAIN_PROMPT_TEMPLATE = """あなたは evolve-anything プラグインの出力品質を評価します。

evolve-anything は Claude Code の「自律進化パイプライン」プラグインで、
...
## 評価基準（ドメイン品質 — evolve-anything 固有）
...
def main() -> int:
    ...
    if delta > 0.15 and good_score > 0.5:
        feasible = True
        conclusion = "✓ 転用可能。evolve-scorer の3軸は LLM 出力評価に機能する。"
        recommendation = "Week 2: Approach A 完了 → evolve-scorer を golden_cases.jsonl の評価に接続"
    elif delta > 0.05:
        feasible = True  # 部分的
    ...
    findings = {
        "spike": "evolve-scorer output eval feasibility",
        "date": "2026-04-16",

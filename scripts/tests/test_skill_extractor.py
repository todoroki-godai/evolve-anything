...
from skill_extractor.decomposition import SAMPLE_TRIGGER_CHAR_LIMIT
...
class TestMachineryFilter:
    ...
    def _meta_turn(self, content, uuid="u", ts="2026-01-01T00:00:00.000Z"):
        """isMeta=True の harness 注入ターン（#322）。"""
        return {
            "type": "user",
            "message": {"role": "user", "content": content},
            "isMeta": True,
    ...
    def test_skips_isMeta_turn_and_finds_real_prompt(self):
        """isMeta=True の harness 注入 system prompt（artifact-design 等の長文含む）を
        飛ばして、その手前の本物の依頼を拾う（#322: 8k字の注入プロンプトが誤って
        user_prompt として抽出され result JSON が肥大化した実例の再発防止）。"""
        injected = (
            "Approach this as the design lead at a small studio known for their "
            "versatility, giving every client a visual identity..."
        ) * 50  # 実例同様の長文注入を模した長さ
        turns = [
            self._user_turn("グラフを作って", uuid="u1"),
            self._meta_turn(injected, uuid="u2"),
            self._command_turn(),
        ]
        prompt = _find_preceding_user_prompt(turns, command_index=2)
        assert prompt == "グラフを作って"

    def test_returns_empty_when_only_isMeta_precedes(self):
        """直前が isMeta ターンしか無ければ空文字（注入プロンプトで trigger を汚さない）。"""
        turns = [
            self._meta_turn("Approach this as the design lead...", uuid="u1"),
            self._command_turn(),
        ]
        assert _find_preceding_user_prompt(turns, command_index=1) == ""

    def test_skips_request_interrupted_and_finds_real_prompt(self):
        """[Request interrupted by user] を飛ばして、その手前の本物の依頼を拾う（#322）。"""
        turns = [
            self._user_turn("認証まわりを直して", uuid="u1"),
            self._user_turn("[Request interrupted by user]", uuid="u2"),
            self._command_turn(),
        ]
        prompt = _find_preceding_user_prompt(turns, command_index=2)
        assert prompt == "認証まわりを直して"
...
# ── sample_prompts の truncate（#322 — 注入プロンプト肥大化対策）────────
# 8k字級の長文（誤って拾われた注入プロンプト等）が result JSON に生のまま
# 埋め込まれるのを防ぐため、1件あたり SAMPLE_TRIGGER_CHAR_LIMIT 文字に切り詰める。


class TestSamplePromptTruncation:
    def test_collect_sample_prompts_truncates_long_text(self):
        long_prompt = "設計方針を検討して" * 200  # SAMPLE_TRIGGER_CHAR_LIMIT を超える長さ
        assert len(long_prompt) > SAMPLE_TRIGGER_CHAR_LIMIT
        records = [
            TrajectoryRecord(
                skill_name="sk", user_prompt=long_prompt, outcome="success",
                session_id="s1", timestamp="t",
            )
        ]
        prompts = _collect_sample_prompts(records, max_samples=3)
        assert len(prompts) == 1
        assert len(prompts[0]) <= SAMPLE_TRIGGER_CHAR_LIMIT + 1  # +省略記号1字

    def test_collect_sample_prompts_keeps_short_text_intact(self):
        records = [
            TrajectoryRecord(
                skill_name="sk", user_prompt="実装して", outcome="success",
                session_id="s1", timestamp="t",
            )
        ]
        prompts = _collect_sample_prompts(records, max_samples=3)
    ...
    def test_extract_skill_candidates_sample_prompts_truncated(self, tmp_path):
        """E2E: extract_skill_candidates の sample_prompts が切り詰められる。"""
        long_prompt = "artifact-design 相当の長文注入" * 300
        turns = []
        for i in range(2):
            turns.append({
                "type": "user",
                "message": {"role": "user", "content": long_prompt},
                "sessionId": f"s{i}", "uuid": f"u{i}-1",
            ...
            turns.append({
                ...
                "sessionId": f"s{i}", "uuid": f"u{i}-2",
                "timestamp": f"2026-01-0{i+1}T00:01:00.000Z",
            })
            turns.append({
                "type": "assistant",
                "message": {"role": "assistant", "content": "done"},
                "sessionId": f"s{i}", "uuid": f"u{i}-3",
                "timestamp": f"2026-01-0{i+1}T00:02:00.000Z",
            })
        pj_dir = tmp_path / "projects" / "test-pj"
        pj_dir.mkdir(parents=True)
        (pj_dir / "s.jsonl").write_text(_make_session_jsonl(turns))

        candidates = extract_skill_candidates(
            projects_root=tmp_path / "projects", min_cluster_size=1,
        )
        assert candidates
        for cand in candidates:
            for p in cand["sample_prompts"]:
                assert len(p) <= SAMPLE_TRIGGER_CHAR_LIMIT + 1

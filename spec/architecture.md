...
Last updated: 2026-05-25 (v1.64.x: auto_memory_runner + meta_quality + constraint_decay + negative_transfer + bounded edit gate)

## コンポーネント構成

```
hooks/                  ← Observe 層（16個 + helpers、LLMコストゼロ）[ADR-002]
  ...
  auto_memory_runner.py ← Stop hook: corrections 直近5件 → L2 memory 候補を非同期生成（LLM 1 call 上限、new-file-per-entry pattern）

...
| PJ | 「成功」の定義 |
|---|---|
| evolve-anything | LLMコール最小化、べき等性保証、既存インターフェース互換 |
...
**evolve-anything への示唆**: 現在の `generate-fitness` が生成する keyword マッチの fitness 関数を、PJ コンテキストを読んでコード生成 → accept/reject データで校正 → 進化、というループに発展させられる。Eureka の「Reward Reflection」パターンは `evolve-fitness` の自然な拡張。
...
**evolve-anything への示唆**: 現在の evaluate() は CoT の4軸（clarity/completeness/structure/practicality）を1回の LLM 呼び出しで一括評価している。これを軸ごとに独立した grader に分離し、各 grader に専用のルーブリックを設けるべき。
...
**evolve-anything への示唆**: 環境の fitness を「1回うまくいった」で判断せず、複数回の再現性（pass^k）で評価すべき。また、LLM コール数（Cost 軸）も fitness に組み込むべき。
...
**evolve-anything への示唆**: Rules/Skills の設定が「Claude に正しいツールを正しい順序で使わせるか」を評価すべき。これは静的テキスト分析では不可能で、実タスク実行（Pattern 1）が必要な理由。
...
**evolve-anything への示唆**: 現在の CoT 評価は汎用ルーブリック（clarity/completeness/structure/practicality）。PJ ごとにルーブリックを動的生成する Constitutional 評価（Pattern 3）の根拠。
...
**evolve-anything への示唆**: 主観的な「良さそう」は信頼できない。テレメトリによる客観測定（Pattern 5）が不可欠。「このスキルを使ったらエラーが減ったか」「修正頻度が下がったか」を実データで測るべき。
...
**evolve-anything への示唆**: 「良い環境」= 個別 Skill の品質ではなく、プラットフォーム全体の基盤品質。Coherence（Pattern 4）と Telemetry（Pattern 5）が基盤品質の測定に対応。
...
**evolve-anything への示唆**: evolve-scorer の発展形として、evaluation 時に「Claude Code がどういう手順でタスクを解いたか」のトラジェクトリも評価に含める。
...
**evolve-anything への示唆**: fitness 関数そのものを最適化可能なプログラムとして扱う。`evolve-fitness` の方向性と一致するが、DSPy のようにベイジアン最適化を適用できれば、ランダムな進化より効率的。
...
**evolve-anything への示唆**: evolve-anything が既にやっている「observe → discover → optimize」のパイプラインは GEPA のパターンそのもの。足りないのは「環境全体のフィードバックシグナル」と「進化対象を Skill 以外に広げること」。
...
| PJ | タスク例 | 検証方法 |
|---|---|---|
| evolve-anything | "discover を実行して構造化結果を出力" | `json_schema` で出力形式検証 |
...
PJ 固有の障害シナリオ:
  evolve-anything:     "LLMコール制限を超えた場合に graceful degradation するか"
...
[Intrinsic Metacognitive Learning (OpenReview)](https://openreview.net/forum?id=4KhDd0Ozqe) は「外部報酬だけでなく、自己内省による学習」を提唱。evolve-anything の `reflect` スキルがまさにこれ — corrections から学習パターンを抽出し、環境を改善する。Pattern 10 はこれを**定量化**する。

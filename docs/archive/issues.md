...
`/evolve-anything:backfill` 実行時、backfill スクリプトは `--project-dir` で指定されたプロジェクトのセッションのみを処理するが、後続の `analyze.py` は `sessions.jsonl` / `usage.jsonl` / `workflows.jsonl` を**フィルタなしで全件読み込む**ため、他プロジェクトのバックフィルデータも混在した分析結果になる。

## 再現手順

1. プロジェクトA で `/evolve-anything:backfill` を実行（例: 339セッション蓄積済み）
2. プロジェクトB で `/evolve-anything:backfill` を実行（例: 170セッション）
...
---
*Submitted via /evolve-anything:feedback*
...
APEX-MEM（arXiv:2604.14362）の概念を evolve-anything メモリシステムに適用する。
...
6ノード型オントロジーによる Event-Centric メモリグラフとして evolve-anything を再設計する。

## 前提条件（このIssueを着手する前に必要なもの）

- fleet Phase 2/3 完成（bin/evolve-fleet evolve-all / audit-all）
...
> ## 実装完了（グローバルスキルとして配置）
> 
> tech-eval の実装方針を検討した結果、**evolve-anything プラグインではなくグローバルスキル（`~/.claude/skills/tech-eval/`）として配置**しました。
> 
> ### 理由
> - tech-eval は「任意の PJ のコードベースを評価する」汎用ツールであり、evolve-anything の4つの柱（自律進化/フィードバック/直接パッチ最適化/fleet）と直交する機能
> - `/evolve-anything:tech-eval` という namespace より `/tech-eval` の方が呼び出しがシンプルで、evolve-anything 非導入プロジェクトでも使える
> 
> ### 実装内容
...
| 保留項目 | 理由 |
|---------|------|
| \`bin/evolve-fleet audit-all --parallel\` | Phase 2 の核だが、growth-state 自然更新で当面代替可 |
...
- ADR-022: evolve-anything fleet 化（Phase 1/2/3 の高レベル設計）
...
\`\`\`
cd ~/tools/evolve-anything
claude
# 中で
/evolve-anything:implement
# このIssueを参照して実装開始
\`\`\`

または \`/evolve-anything:implement <issue#>\` でこの issue を直接指定。
...
### 背景
「PC環境を見直して」「全PJの状態をチェックして」のような環境レビュー要求時、現状 evolve-anything は env_score / issues_summary / SUBAGENTS_30d は集計できるが、**LLM トークン使用量（input / output / cache_creation / cache_read）の PJ 別・グローバル集計機能がない**。コスト視点でのボトルネック PJ 特定や、肥大化スキル・暴走 subagent の発見ができない。

なお既存の `subagents.jsonl` の "token-load" 集計は subagent 起動回数であり、LLM トークン量ではない（命名が紛らわしい）。

### 提案
1. `~/.claude/projects/<pj>/*.jsonl` の transcript から PJ × 期間 × トークン種別を集計するユーティリティ追加（例: `bin/evolve-fleet tokens` または `bin/rl-tokens`）。
2. `bin/evolve-fleet status` の表に `TOKENS_30d` 列（または cache hit率）を追加。
...
- rtk gain（RTK 側）が CC の節約量を見るのに対し、こちらは「evolve-anything 環境全体の LLM コスト健康度」を見る役割
...
ブランチ: \`feat/token-usage-tracking\` (worktree: /tmp/evolve-anything-token-usage)
設計ドキュメント: \`~/.gstack/projects/evolve-anything/todoroki-main-design-20260509-082840.md\`

## 実測値 (M1 / evolve-anything PJ 1 個 / --days 7)
...
- **evolve-anything PJ 1 個 / --days 7 を 60 秒以内** ← 必須、CI bench 化
...
- worktree: \`/tmp/evolve-anything-token-usage\` 残存
...
3. \`/evolve-anything:implement\` で worktree 内に実装
...
Claude Code / evolve-anything レベルで以下のような制御ができると嬉しい：
...
- 疑い: LLM 統合系（fitness 評価・evolve-loop 系）でモック漏れ／実 API call、DuckDB ingest / token_usage_store で実 walker、fixture が `~/.claude/projects/` を実走
- mock 化 or fixture 局所化で短縮（目標: 全体 30s 以内）

### B. 不要 skill の棚卸し
- evolve-anything プラグインが提供している skill 一覧を再評価
...
evolve-anything は現状 `prune` スキルの LLM judge で都度判断しており、**fitness 関数に skill 肥大化への明示ペナルティが無い**。
...
- skill 増殖時に environment score が自動で下がり、`/evolve-anything:audit` で早期検知できる
...
evolve-anything は observe hooks (15個) で **tool 呼び出しを既に全量記録している**が、この signal を skill 品質の評価に back-prop していない（現在は telemetry の使用回数・成功率レベルにとどまる）。
...
- observe hooks のログは既に `~/.claude/evolve-anything/` に蓄積（`token_usage_store` と同じ系統）
...
evolve-anything は実質的に同じ設計（Claude Code 自体は frozen、plugin 側のスキル群と curation ロジックだけが進化）を採っているが、**設計判断としては明示化されていない**。SkillOS は同じ分離設計を独立に発見・正当化した先行研究として引用価値が高い。
...
>
> - Claude Code 本体（executor）は frozen と扱い、evolve-anything は curator 側の品質（skill 群と curation ロジック）のみを進化対象とする
...
- evolve-anything の scope が agent executor 自体の改善に拡張された場合は ADR を superseded にする
...
論文の手法詳細・査読コメント・evolve-anything との対応関係・取り入れる優先順位の全文はこちらを参照。
...
- evolve-anything は public/todoroki-godai org → push / PR とも \`todoroki-godai\` で固定
...
evolve-anything の `hooks/stop_failure.py` は現在 `error_type` フィールドに `rate_limit` / `auth_failure` / `unknown` 等の**技術エラーのみ**を記録しており、エージェント行動レベルの失敗分類は存在しない。これにより `reflect` スキルが「何を学んだか」を分類できず、学習効率が頭打ちになっている。
...
evolve-anything の現行パイプラインは偶然にも同型の構造を持っている：
...
しかしこの対応関係は**設計判断として明示化されていない**。また MemOS が解決している「層間の矛盾解消（HiMem の conflict-aware reconsolidation）」が evolve-anything では手動になっている。

## 提案

1. **ADR を追加** (`docs/decisions/ADR-XXX-memory-crystallization.md`)
   - evolve-anything の 4 層メモリ設計を明文化し、MemOS との対応関係を根拠として示す
...
evolve-anything の `corrections.jsonl` は現在、修正の「内容」は記録しているが、**修正直前のツール呼び出し列（コンテキスト）を記録していない**。このため `reflect` スキルが「どのツール操作の後に誤りが起きたか」を分析できず、pitfall 生成の精度が低い。
...
evolve-anything の audit や evolve フローの中で、プロジェクトの言語構成（Python/TypeScript 等）を検出し、Claude Code の LSP サポート（.lsp.json + 言語サーバー）の導入を提案する機能がほしい。
...
evolve-anything への採用推奨度「高」と判定した。
...
🔴 要対応 (1件)
  - /evolve-anything:reflect — 未処理フィードバック 9件
...
**方針**: 判定カード形式に変更
```
🔴 要対応: /evolve-anything:reflect — 未処理フィードバック 9件
...
## #185 feat(evolve): プラグイン自己診断ギャップ — evolve-anything 本体スキルを skill_evolve / Pitfall剪定 / Fitness Evolution 対象にする  `[closed]`  (enhancement)

## 問題

evolve-anything を **evolve-anything PJ 内で実行**すると、以下のフェーズが常にスキップされる:
...
# evolve-anything 本体の skills/ ディレクトリ（プラグインスキル）は対象外
```

evolve-anything には `.claude/skills/` が存在しないため、カスタムスキル = 0件。  
`skills/evolve/`, `skills/reflect/` 等のプラグインスキルは `origin = "plugin"` として扱われ除外される。

### 2. Fitness Evolution のデータ蓄積条件が未達

Fitness Evolution は `~/.claude/evolve-anything/optimize*.jsonl` の accept/reject 実績を使う。  
カスタムスキルがないため `optimize` を実行できず → データが溜まらない。

### 3. 構造的矛盾

evolve-anything はスキルを `skills/<name>/SKILL.md` に配置するが、  
evolve が読む対象は `.claude/skills/<name>/SKILL.md`（ユーザー自作スキル）。  
プラグイン本体が自分自身のスキルを改善できない。

## 影響

- evolve-anything PJ を長期間使い続けても Pitfall / Fitness のデータが蓄積されない
- 「なぜ 0件なのか」がユーザーに伝わらず混乱を招く
- evolve-anything 自身のスキル品質を evolve で改善できない
...
evolve-anything PJ 専用に `CLAUDE_PLUGIN_SELF_DIAGNOSE=true` 等の環境変数で有効化。

### Option B: evolve 出力に「なぜスキップされたか」を明示（即時・低コスト）

現在の「カスタムスキル 0件のためスキップ」メッセージに、  
「evolve-anything プラグイン本体を diagnostics に含めるには Option A を参照」等の説明を追記。  
少なくともユーザーの混乱を防ぐ。
...
セッション中に `/evolve-anything:evolve` を実行した際、  
...
> 
> **未解消（根本バグ残存）:**
> `skill_evolve_assessment` / `pitfall_hygiene` が使う `find_artifacts()` 本体（artifacts.py:61-65）は依然 `.claude/skills/` のみ参照 → evolve-anything PJ で evolve を実行するとプラグインスキルが 0 件扱いのまま。
> 
...
Library Drift (arXiv:2605.19576)・HASP (arXiv:2605.17734) の2論文が evolve-anything の現行実装と直接対応する課題を指摘している。

**核心問題**: evolve-anything は「スキルが増える仕組み（evolve/discover）」は持つが、「スキルが腐敗する仕組みを検知・修復する」定量基盤がない。Library Drift 論文では LLM 自動生成スキルが人間キュレーション比 -16.2pp の性能差を生む「Library Drift」を実証しており、現状の prune スキル（ゼロ invocation 検出のみ）では不十分。
...
HASP 論文（arXiv 2605.17734）が示す「エージェントが失敗しやすい状態で自動発動するガードレール」の設計を evolve-anything に取り込む。
...
evolve-anything の現行 flat memory を3層階層に進化させ、`reflect` スキルのセッション横断精度を上げる。
...
- `/evolve-anything:second-opinion` で agentmemory のアーキテクチャを評価してから設計
- agentmemory の benchmark 手法を `evolve-scorer` の評価軸に取り込めるか確認
...
- **Before**: evolve-loop 結果を1件ずつ見ても繰り返しパターンに気づけない
...
- **確認方法**: `/evolve-anything:audit` 出力に「繰り返し失敗パターン」セクションが現れ、同一 skill で 3 回以上繰り返されたパターンが列挙されること
...
Microsoft Research の SkillOpt (arXiv 2605.23904) が提案する「エグゼクティブ層によるスキル廃棄」を evolve-anything の `evolve-skill` / `prune` に組み込む。現在、`prune` は手動トリガー型で自動廃棄条件が定義されていない。低品質スキルが蓄積すると、AI が古いロジックを踏み続け学習ループの精度が劣化する。
...
- [ ] `evolve-anything:evolve-skill <skill>` 実行後に `retirement_candidate` フィールドが SKILL.md metadata に出力されること
- [ ] `evolve-anything:audit --coherence-score` で廃棄候補スキルがリストアップされること
...
arXiv 2605.06445 (Constraint Decay) が実証した「長期タスクで LLM が制約を徐々に忘れる現象」に対し、evolve-anything のスキル実行ループおよび figma-to-code の `evaluate:human` に制約持続性スコア (GPS 相当) を導入する。
...
3. evolve-anything の `evolve-loop-orchestrator` に「constraint_decay_fold が空でない場合はスキル blurb を強化して再実行」トリガーを追加
...
## 背景・問題
`fitness_evolution`（適応度関数の重みを人間フィードバックとの相関で進化させる機構）が、サンプル不足（0/30件）で実質デッドフィーチャー化している。母集団が `optimize`/`evolve-loop` の accept/reject イベント（`history.jsonl` の `human_accepted is not None`）に限定されており、「1日1回 evolve」程度の運用では永遠に貯まらない。
...
## 実装スコープ（最小 PR）
- (a) `fitness_evolution.py` の `insufficient_data` メッセージに「optimize/evolve-loop/evolve diff 提案の accept/reject が母集団」と明記。`best_fitness=None` を相関母集団に入れないガード（現 L82 付近）をテストで固定。
...
Co-ReAct (arXiv 2605.23590) の知見を evolve-anything の evolve フローに適用する。
...
CoSPlay (arXiv 2605.23491) の「テスト時自己対戦」パターンを evolve-anything の evolve フローに適用する。
...
python3 -m evolve_anything evolve --dry-run <skill-name>
...
> 
> **正しい実装案**: evolve-scorer の pre-application 自動呼び出し（haiku/sonnet コストが発生）
> 
> **TODO before implementation**:
> - [ ] evolve-scorer を pre-application で自動呼び出しするコスト見積もり
...
- **Before（自己モデルなし）**: evolve-anything 自身が自分のどの軸が弱いか把握していない。evolve は人間の手動指示のみ
...
> #### DuckDB テーブル設計
> 
> `~/.claude/evolve-anything/token_usage.db`（既存 DB に追加）に以下テーブルを追加する。
...
> #### 自己モデルデータ構造
> 
> `~/.claude/evolve-anything/self_model.jsonl`（JSONL、append-only）
> 
> ```json
...
>     "action_hint": "/evolve-anything:audit --fitness telemetry"  # [NEW] 優先 audit コマンド
> }
> ```
> 
...
>  推奨: /evolve-anything:audit --fitness telemetry を先に実行」
> ```
> 
> #### 定期 audit 設計
> 
...
>         TriggerResult(
>           triggered=True,
>           action="/evolve-anything:audit --fitness telemetry",
...
> ### 前提確認
> 
> `investigate` スキルは evolve-anything には存在しない（gstack プラグイン側のスキル）。
...
> ### 仮説ツリー データ構造
> 
> **ファイル名規則**: `~/.claude/evolve-anything/hypothesis_{session_id}.jsonl`
> 
> ```json
> {
>   "hypothesis_id": "h1",
...
> - セッション終了後に cleanup スキルの対象になれるよう `evolve-anything-` プレフィックスを付ける
>   → `~/.claude/evolve-anything/hypothesis_rl-anything-{session_id}.jsonl`
> 
> ---
> 
...
> 
> `investigate`（gstack プラグイン）は evolve-anything 外のスキルだが、設計方針は共通化できる。
> 
> **共通化できる部分:**
...
## 出典
ai-daily-report 2026-05-29 / arxiv:2605.28814（evolve-anything: fitness 関数設計に直結と評価）
...
## 出典
ai-daily-report 2026-05-29 / arxiv:2605.28732（evolve-anything のメモリ設計改善に直結と評価）
...
## 提案アクション
Embodied-Minds-Lab/BES の forward search 実装を読み、進化演算子を `genetic-prompt-optimizer` / `evolve-loop` に試験導入。

## 採用後の確認方法
- [ ] 同一ベースラインで 1パス版 vs 進化演算子版を evolve-loop で比較 → best-case スコアが上回るか
...
## Decisions
- ai-daily-report (2026-05-29) の tech-eval から evolve-anything 直結4概念を Issue 化 → agent team で実装。
...
- AEPO（探索型ポリシー最適化）: RL訓練前提で evolve-anything 設計と不適合のため scope 外。
...
5. マージ後 `/evolve-anything:cleanup` で locked worktree 3つ片付け。
6. spec が変わるため `/evolve-anything:spec-keeper update` 検討。
...
mattpocock/skills の tech-eval から、evolve-anything に欠けている「開発タスク着手直前」レイヤーの2概念を提案する。meta層（evolve/audit/prune/triage/handover/skill-creator）は上位互換で取り込み不要だが、この2つは mattpocock の看板価値と一致しギャップが大きい。
...
- **確認方法**: 数セッション運用後 `bin/evolve-fleet status` の correction 傾向で「意図ズレ/手戻り」系 correction 発生率が低下しているか
...
- **確認方法**: 同一タスクを用語集あり/なしで実行し `bin/evolve-fleet tokens` で input/thinking トークンを比較
...
## 採用後の確認方法
- [ ] `/evolve-anything:evolve`（または audit）を回す → memory セクションに「block した低信頼要約 N 件」が出る
...
## 概念（再フレーム済）
論文 Self-Trained Verification（arXiv:2605.30290）は「外部ラベル無しで検証器を自己学習」する手法。evolve-anything は決定論 Python + LLM 呼び出しのみで **ML 学習基盤を持たない**ため、額面通りの検証器学習は innovation token の浪費・設計不整合。
...
## NOT in scope
- 論文通りの「学習する検証器モデル」（ML学習基盤は evolve-anything に不適合 → 却下）
...
自己進化ハーネスにおいて「更新したこと自体」と「実際に効いた更新」を切り分ける ablation 方法論を提案する論文。evolve-anything の根幹（「更新したが効いていない自己進化」の検出）に直撃する。

daily report (2026-06-02) では evolve-anything ⭐⭐⭐ 判定だが、tech-eval で実コード照合した結果、トレンド9概念中6つは既に実装済み。本件は数少ない「実装にギャップがある」概念。
...
- [ ] \`/evolve-anything:audit\` を回す → observability セクションに「更新コンポーネント別 delta」行が surface される（audit だけで出ること）
...
日報 (2026-06-03) の ⭐⭐⭐ 概念 **SIRI**（arXiv 2606.02355: 成功軌跡からスキルを採掘→検証→コアポリシーへ蒸留）を tech-eval で照合した結果、evolve-anything には **採掘モジュール `skill_extractor` が実装済み（Issue #238 Phase 1）だが、どの recurring ループにも配線されていない**ことが判明した。
...
## SIRI の3段階と evolve-anything の対応

| SIRI 段階 | evolve-anything 既存 | 状態 |
|-----------|------------------|------|
...
- [ ] `/evolve-anything:discover`（または `/evolve-anything:evolve`）を回す → レポートに「成功軌跡から採掘したスキル候補（N 件）」が出る。出なければ配線が効いていない。
...
日報 (2026-06-03) の概念 **TASTE**（arXiv 2605.28556: ツール呼び出し列から難問を逆生成し、既存エージェントベンチの飽和を暴く）を tech-eval で照合した結果、evolve-anything の eval 生成は**順生成のみ**で、**eval 自体の飽和（緑なのに頑健でない）を検出する経路が無い**ことが判明した。
...
- [ ] `/evolve-anything:audit` を回す → observability セクションに「eval 飽和度」行が calibration drift と並んで出る。
...
> 
> ---
> *Correction submitted via /evolve-anything:feedback*
...
evolve を回した後、その結果のなかに evolve-anything 自身のバグや改善余地が見つかることがある。現状これを拾うフローは **完全に手動** で、ユーザーが気づいて `/evolve-anything:feedback` を叩いて初めて issue 化される。

直近の実例が #295: evolve の dry-run が shadow コピー環境で実行された際、対象 PJ の CLAUDE.md を解決できず（`No CLAUDE.md found, skipping missed skill detection`）、CLAUDE.md 依存の除外ロジック（`detect_untagged_reference_candidates` の「Skills セクション記載スキルは除外」など）が軒並み無効化され、`untagged_reference` / `missed_skill` / prune zero-invocation の誤検知が大量発生した。これは「沈黙＝問題なし」ではなく「環境解決失敗による誤検出」で、ユーザーが手動で1件ずつ切り分けたうえで `/evolve-anything:feedback` して #295 を起票した。
...
- 既存の trigger 提案（`trigger_engine`）や `/evolve-anything:feedback` と同型の安全設計（提案 → 承認 → 実行）にする。
...
- **feedback スキル**（`skills/feedback/SKILL.md`）: 起票経路を再利用する。テンプレ（カテゴリ / コンポーネント / 詳細）、ラベル `feedback`、`gh issue create --repo todoroki-godai/evolve-anything ... --label "feedback"`、認証チェック（`gh auth status`）、未認証時の `~/.claude/evolve-anything/feedback-drafts/` ローカル保存フォールバック、プライバシー保護（SKILL.md 内容・ローカルパス・PJ固有情報を含めない MUST NOT）をそのまま踏襲する。
...
- issue ドラフトの自然文整形に LLM を使う場合は、`/evolve-anything:feedback` と同じく承認ゲートを通す。バッチで多数のドラフトを LLM 生成する設計にする場合は、`.claude/rules/llm-batch-guard.md` 準拠で件数・見積もりトークン数を事前にユーザーへ提示し確認を取る（無確認のバッチ LLM 処理は禁止）。
...
- [ ] 起票経路は feedback スキルを再利用し、ラベル `feedback`、リポジトリ `todoroki-godai/evolve-anything`、未認証時のローカルドラフト保存、プライバシー保護（SKILL.md 内容・ローカルパス・PJ固有情報を含めない）を満たす。
...
現状、`/evolve-anything:evolve` は Observe → Diagnose → Compile → Housekeeping → Report の3ステージを実行するが、**evolve 自身の実行結果（提案の質・誤検出・実行時エラー）を振り返る経路が無い**。
evolve が出した提案にバグや改善余地があっても、それは人間が気づいて手で issue を立てるまで構造に残らない。「install ≠ enforcement」（自動で回るループに載っていないものは育たない）と同型の配線漏れになっている。

そこで **evolve 実行後に evolve の出力を自己解析し、バグや改善点を検出したら evolve-anything 自身にフィードバックして GitHub issue を半自動で起票する**機能を追加したい。自己進化パイプラインのメタ層（パイプライン自身を改善するループ）を閉じるのが狙い。
...
<!-- evolve-introspect:self:split_archive_contradiction:onboard-project -->
...
<!-- evolve-introspect:self:split_archive_contradiction:project-setup -->
...
## 採用後の確認方法
- [ ] `/evolve-anything:evolve`（または audit）を回す → reorganize/prune セクションに「階層統合提案（低レベル→上位）」行が出る
...
## #304 [tech-eval] Skill-RM: スキル軸での異種評価基準統一（報酬モデル）を evolve-scorer/fitness に検討  `[closed]`  (enhancement)

## 概念
Skill-RM (arXiv:2606.03980) は、タスクごとに異なる評価基準を「エージェントのスキル」を共通軸として単一報酬モデルで横断評価する。現状の evolve-anything fitness は「軸別」重み統合（coherence/telemetry/constitutional/skill_quality）で、「スキル別」の異種基準統一とは**直交**する。

## Before / After（運用者体験）
- Before: fitness は軸別重み統合。スキルごとの異種成功条件は統一スコアで測れない
- After: スキルごとの異種基準を1つの報酬で横断評価でき、evolve-scorer のキャリブレーションが安定（🛡安定性）

## 既存実装との差分（根拠）
- `scripts/rl/fitness/environment.py:71`（`_normalize_weights`）/ `:80`（`compute_environment_fitness`）— 軸別の動的重み統合
- evolve-scorer は技術40%/ドメイン40%/構造20%
- → 「スキルを共通軸にした評価統一」は未実装

## 配線先
evolve-scorer / environment fitness（evolve のたびに発火）

## 採用後の確認方法
- [ ] `/evolve-anything:evolve` を回す → evolve-scorer 出力にスキル別の統一スコアが現れ、calibration drift の乖離が縮小
...
## 概念
Microsoft SkillOpt（Rohan Paul 解説, daily report 2026-06-04）は「agent skill は手書き・LLM一発生成・緩い修正で劣化しやすい」と問題提起し、スキルを**小さな外部プログラムとして訓練**すべきと主張。evolve-anything の optimize は LLM 1パスパッチ＋regression gate で、勾配的な「訓練」とは異なる。
...
## 配線先
optimize / evolve-loop `--evolve-search`（evolution_operators が消費）

## 採用後の確認方法
- [ ] `evolve-loop --evolve-search` を回す → 世代ごとの subgoal fitness が単調改善し、収束世代数が減る
...
## 概念
"What Makes Interaction Trajectories Effective for Training Terminal Agents?" (arXiv:2606.03461) は、端末エージェント訓練に**有効な相互作用軌跡の条件**を実証的に調査。evolve-anything の skill_extractor は generalizability_score の閾値（TRAJECTORY_SKILL_SCORE_THRESHOLD=0.25）で機械的にフィルタしており、score 算定根拠を補強できる。
...
## 採用後の確認方法
- [ ] `/evolve-anything:discover` を回す → trajectory_skill_candidates の triage 通過率（採用/却下比）が改善
...
- [ ] `/evolve-anything:evolve` を**連続2回**回す → 2回目で同じ SKIP 候補が個別表示されず「SKIP 抑制 N件 ✓」に畳まれる（①）
...
## やること
- live registry（gstack skills + evolve-anything skills + ~/.claude/skills + 各プラグイン SKILL.md）の skill 名集合を構築
- **正規化レイヤーを先に固める**: `/evolve-anything:spec-keeper update` → `spec-keeper` 等のプレフィックス/ツール名/空白除去。テストで変換を固定してから検出に繋ぐ
...
- evolve-anything の observe → diagnose 構造に乗せ、follow-through が低い hook は delete 候補として surface
...
3. `~/.gstack/flow-chain.json` は `/evolve-anything:implement` `/evolve-anything:spec-keeper update` を参照する **手動メンテのファイル**（gstack 純正ではない）。`gstack_version` フィールドは手書きのピン。
...
## 採用後の確認方法
- [ ] 配線後: `/evolve-anything:audit` を回す → レポートに「エージェント編成ギャップ」section が出る。
- [ ] `/evolve-anything:agent-brushup` でドメインを渡す → チーム定義 diff が提示され、承認するまで subagent は起動しない（subagent-guard 整合の検証）。
...
sys-bots で `/evolve-anything:evolve` を1回フル実行した際、出力の再現性・パース可能性に2件の不具合があった（報告: todoroki-godai/sys-bots, 2026-06-05）。

### 1. stdout に警告ノイズと結果 JSON が混在（パース不可）
`evolve --dry-run` の stdout 先頭に診断ノイズが出力され、その後ろに本体の結果 JSON が続く:
...
## 環境
- evolve-anything plugin / sys-bots PJ（env_tier: large, スキル14 custom + 5 archived）
...
sys-bots で `/evolve-anything:evolve` を実行したところ、remediation / discover の検出が前処理フィルタ欠落により誤検知まみれになった（約80%ノイズ）。共通根は「アーカイブ除外・doc文脈ID除外・ストップリスト」の欠落（報告: todoroki-godai/sys-bots, 2026-06-05）。
...
## 環境
- evolve-anything plugin / sys-bots PJ
...
<!-- evolve-introspect:stale-ref-ssm-tmp-autofix -->
...
<!-- evolve-introspect:reorganize-scipy-nan -->
...
<!-- evolve-introspect:self-analysis-blind-warnings -->
...
手動止まりにしない。**evolve-fitness / `optimize_history_store` → audit の `calibration_drift` section** に配線し、`/evolve-anything:evolve` のたびに自動発火させる（version ≠ enforcement）。

## 採用後の確認方法

- [ ] `/evolve-anything:evolve` を回す → report の `calibration_drift` 帯に「軌跡選好で再較正した fitness の accept 予測精度」が section として出る。
...
- [ ] `/evolve-anything:evolve` を回す → calibration_drift 帯に pairwise 報酬で再較正した accept 予測精度が **既存 binary 較正と並んで**出る（A/B 比較可能な形）
...
> - evolve は較正データを一切書かない（writer は手動 `/optimize` / `/evolve-loop` だけ）
> 
...
<!-- evolve-introspect:self:fp_in_auto_fixable:extensionless_logical_path:data/bots/wheeling -->
...
evolve-introspect:prune_zero_invocation_telemetry_dead
...
evolve-introspect:hardcoded_value_doc_url_overfire
...
sys-bots の ev-0607 セッションで `/evolve-anything:evolve` を回しても、#356 のゲート条件である **calibration regression の証拠がゼロ**。調査したところ:

- `optimize_history/`（accept/reject 母集団）ディレクトリが **全PJで存在しない**（`~/.claude/evolve-anything`・`~/.gstack` 両方で空）
...
| 経路 | 場所 | 駆動条件 |
|---|---|---|
| writer① | `skills/genetic-prompt-optimizer/scripts/optimize.py:535` `record_human_decision(human_accepted=args.accept)` | `/evolve-anything:optimize` を `--accept`/`--reject` で手動実行したときだけ |
| writer② | `skills/evolve-loop-orchestrator/scripts/run_loop.py:749` `_history_store.append_entry(loop_result, ...)` | `/evolve-anything:evolve-loop-orchestrator` を手動実行したときだけ |
...
- [ ] `bin/evolve-fleet` 等で全PJの `optimize_history/<slug>.jsonl` 行数を表示 → 全ゼロを確認
- [ ] `/evolve-anything:evolve` を回す前後で `optimize_history` が増えないことを確認（writer 非配線の実証）
- [ ] `/evolve-anything:optimize --accept` を1回実行 → optimize_history が増えることを確認（writer は生きているが手動限定の実証）
...
- **hook 実行時**（env `CLAUDE_PLUGIN_DATA` 有）→ plugin-data dir `~/.claude/plugins/data/evolve-anything-evolve-anything/`
- **tool/skill 実行時**（env 無）→ fallback `~/.claude/evolve-anything/`
...
## やること（設計要件）
1. **正準 dir を決める**: reinstall 耐性のため **fallback `~/.claude/evolve-anything/` を正準**にし、plugin-data → fallback の **逆 migration**（fallback は再 clone でも残るが plugin-data は uninstall で wipe される）
...
## 補足
発見元: evolve SKILL.md progressive disclosure リファクタ後の実機検証（evolve-anything 自身）。同時発見の Step 6 dead reference / glossary 汎用語 / agent_team 過剰警告は別 PR で修正済み。
...
> 
> - `/evolve-anything:evolve` を回すたびに discover の matched_skills(skill diff) + skill_evolve(high/medium) の accept/reject が `optimize_history` へ自動記録され、fitness_evolution の母集団になる（`evolve_decisions.py:111-137`）。＝特別な操作は不要で、evolve を回し続けること自体が母集団を貯める。
> - insufficient_data の案内文が #360-A 以前の手動導線（evolve-optimize で accept しろ）のままだったため、「evolve を回せば自動で貯まる」を明示。
...
- [ ] `/evolve-anything:evolve`（または `discover`）を回す → スキル候補テーブルに `routing`（配線先）と `attachments`（必要リソース）の列が現れ、`generalizability_score` だけでなく「どこで発火するか」が候補ごとに表示される。
...
#381 (PR #383) で `skill_extractor` に Workflow-to-Skill の4軸構造分解（`decomposition.py`）を入れた。マージ後、実 PJ（evolve-anything、169 transcript files → `max_files=50` サンプリング）で本流経路（`run_discover` → `extract_skill_candidates`）を E2E 実走したところ、4軸自体は正しく動作したが、`routing.trigger_keywords` に**ノイズ語**が混じることが判明した。
...
| `evolve-anything:spec-keeper` | `["if", "gstack", "claude", "not", "review"]` |
| `evolve-anything:implement` | `["review", "gstack", "if", "plan", "not"]` |
...
- 実行: `/evolve-anything:evolve`（docs-platform, 2026-06-09）
- env_tier: medium
...
## #395 [Feedback] evolve の doc/出力構造の乖離2件: evolve.py vs evolve / skill_evolve 出力の二重構造  `[closed]`  (documentation,feedback)
...
### 1. SKILL の `evolve.py` 直叩き前提が実環境（`evolve` ラッパー）と乖離
...
`evolve.py` の実パスがドキュメントから分からず、glob 探索（`~/.claude/plugins/*/evolve-anything/scripts/evolve.py` 等）が空振りした。最終的に `evolve --confirmed-batch ...` ラッパーで動作した。

**期待**: SKILL の再実行手順を `evolve --confirmed-batch ...`（インストール時に PATH に入るラッパー）に統一するか、両方を明記する。「evolve.py を直接探せ」という前提を外す。
...
#400 で `evolve_decisions`（ADR-041）の dry-run 運用経路を根治し、PR #401 で **emit→apply→ingest→fitness reader** が apply 境界をまたいで動くことを E2E + 実 store パスで実証した（空の本物 store が 0→1、worktree-safe slug `evolve-anything` で reader が独立に同一パスを解決して観測）。
...
2. もしくは `evolve --drain`（emit と対の CLI）を用意し、SKILL.md からは1コマンド呼ぶだけにして「呼ばれたか」を fire-log で計測（hook_drift の follow-through 計測と同型）。
...
## 実測（figma-to-code, `evolve --dry-run`）
...
- evolve-anything（plugin cache 1.92.x 系）
...
- SKILL Step 1 が `evolve --dry-run --output /tmp/rl_evolve_out.json`（**PJ 非依存の固定パス**）を指示。
...
#364（PR #414）の `evolve-fleet migrate-data` が rebuild 方式マージで一度 compaction するが、**migration 後も hook の書き込みパターンは per-fire のままなので数ヶ月で再肥大する**。
...
`evolve-fleet status` の ISSUES 列が全 PJ で 600 前後に揃っており（bots/receipt/figma-to-code は**ぴったり 599 で一致**）、env_score の物差しとして死んでいる。再現調査で 599 件を完全再現し、根因3点を確定した。
...
- [ ] 実環境で audit 再実行 → evolve-anything の hardcoded_values が 599 → 1桁台に減少することを実測
- [ ] fleet の同値カウント警報テスト
...
- evolve_decisions: evolve-anything/docs-platform **0 件**、sys-bots 2 件のみ
- evolve_pending: 空
...
2. **SessionStart 自動 drain**: `evolve --drain`（apply 済み提案の accept 判定回収）を restore_state hook に同居させ、人手ゼロで決定論回収。learning_skill_md_must_not_enforcement の教訓（SKILL.md の MUST では実行され損ねる）をそのまま適用し、「実行され損ねない場所」に置く
...
exact model ID（例: `claude-opus-4-8`）を pin すると、新モデルリリース後も古いモデルに固定されたまま気づけない **silent stale** が起きる。実際に global agents 4件（design-review / refactor-engineer / senior-engineer / reviewer-san）で発生していた（手動で修正済み）。これは `evolve-fleet plugins` が検出する「version 無しプラグイン silent stale」と同型の問題。
...
## #460 [tech-eval] evolve-fleet recall に [[link]] 1-hop 展開を追加（Organize then Retrieve 部分採用）  `[open]`

## 概要

arXiv 2606.11680 (Organize then Retrieve: 階層メモリ検索) の tech-eval（2026-06-12 daily report 照合）で特定したギャップ。
evolve-anything の memory は**保存側は階層化済み**（MEMORY.md index → fact file → `[[link]]` 相互リンク）だが、**検索側 `scripts/lib/fleet/recall.py` はフラット TF スコアリングのみ**で階層を活用していない。hit した fact の `[[link]]` 先を 1-hop 展開して併記すれば、決定論のまま「芋づる想起」が手に入る。

## Before / After（ユーザー体験）

- **Before**: `bin/evolve-fleet recall "duckdb checkpoint"` がキーワード一致 fact を単発で返す。関連 memory（リンク先 pitfall 等）は人が `[[link]]` を見て手で開く
...
`bin/evolve-fleet recall`（オンデマンド CLI）。recall は「思い出したい瞬間」に叩く性質のツールなので**手動発火が本来形**であり、recurring ループ（evolve/audit）への配線は不要と判断済み。

## 採用後の確認方法

- [ ] `bin/evolve-fleet recall "duckdb checkpoint"` → hit した fact に `↳ linked:` で `[[link]]` 先（関連 pitfall 等）が併記される
- [ ] `bin/evolve-fleet recall "duckdb checkpoint" --json` → 各 hit に `linked` フィールドが付く
...
- weak_signals 318 件の PJ 内訳: figma-to-code 116 / amamo 48 / evolve-anything 47 / atlas-breeaders 24 / receipt 20 …
...
- evolve SKILL.md Step 6.1/6.2 にも `confirm_idioms` への言及なし（「はい」→ `evolve-reflect --promote-weak` → `record_reviewed` のみ）
- `evolve-reflect --promote-weak`（reflect.py:747）も `promote_signals` のみで confirmed 化しない
...
2026-06-12 の初回 bootstrap（evolve-anything、30件承認）では、provenance（source_path+line_no）突合で signal_key→idiom_key を解決し `confirm_idioms(confirmed_by="bootstrap_review")` を手動呼び出しで補完済み（confirmed 30件）。**他 PJ の今後の daily_review では再発する。**

## 修正案

1. **配線**: 昇格と confirmed 化を1経路に束ねる。推奨は `evolve-reflect --promote-weak` 内で promote 成功後に当該 signal の idiom を confirmed 化する（SKILL の散文に新ステップを足すのではなく CLI に寄せる — ADR-045 の enforcement 原則と同じ「tool 文脈に閉じる」）。signal→idiom の対応は provenance（source_path+line_no）突合（本 issue の暫定処置と同じロジック）をライブラリ化
...
2. **帰結: prune zero_invocation が構造的 FP**: 上記により prune の zero_invocation 候補（今回 custom 3件）が構造的に false positive 化する。今回は SKILL.md 全文 + git log の個別調査で全件「オンデマンド型=keep」と判定できたが、候補抽出の根拠データ自体が壊れていると毎回の調査コストが無駄になる。skill_evolve の insufficient_usage（usage_count==0 で保留）判定にも同じデータが使われており影響が広い。
   - 確認観点: Skill tool 発火が usage-log に流れる経路の有無、`evolve-usage-log` 手動呼び出しに依存していないか
...
## 証拠（read-only 実測）
- 同一検出コードを実走: `weak_signals.batch.run_batch('evolve-anything', dry_run=True)` → `{'detected': {'permission_deny':5, 'esc_interrupt':9, 'manual_edit_after_ai':8, 'rephrase':1}, 'written':18, 'skipped_dup':5}` — **18件が今この瞬間「未保存」**（既存 store に無い）
- 検出経路は健全: slug 解決一致（`pj_slug_from_cwd`='evolve-anything'、evolve.py:1179 `_resolve_pj_slug` と同値）、transcript 60件取得成功
...
## 証拠
- `which` / `--help` 実行 → 3本とも NOT FOUND。`bin/` の executable 15本に含まれず（`bin/evolve-backfill-turn-indices` は別物）
...
## 修正案
backfill の取り込みは v1.96.0 以降 session_store / utterance_archive の batch ingest（hooks 自動 + `bin/evolve-fleet ingest`）に置き換わっている。
- (a) SKILL.md を現行の取り込み経路（`evolve-fleet ingest` 等）に書き換える、または
...
- 実測: dry-run で `~/.claude/evolve-anything/evolve_pending/<slug>.json` added / 事前 seed が削除。戻り値 `persisted: false` が誤誘導
...
> - 実測: usage.jsonl に `feedback` 46件 / `bots` 45件（evolve-anything / sys-bots の worktree セッション由来）
...
## 3. doc 軽微 stale 3点
- `skills/audit/SKILL.md:75-80` — `from skill_usage_stats import` 例に sys.path 前置なし（補助スニペット・主経路は evolve-audit CLI で完結）
...
> 💬 comment:
>
> 追加 LOW（後片付け中に発見）: テストが実 /tmp に `evolve-anything-last-skill-sess-proj-002.json` を漏らしていた（2026-06-12 フルスイート実行時刻と一致）。writer は `scripts/lib/rl_common/workflow.py:89-92 last_skill_path` = `TMPDIR` env または /tmp。設計自体は意図的（cross-process の ephemeral marker）だが、last_skill 系テストの一部が TMPDIR を tmp_path に monkeypatch していない。該当テスト（hooks/tests/ の last_skill 利用箇所）に TMPDIR 隔離の autouse fixture を足す。
...
> 
> 本 issue の対応時に audit/SKILL.md:75 と合わせて4件まとめて修正すること。修正後は `bin/evolve-dogfood-gate --layer 3` が緑になることが受け入れ基準。
...
> - `bin/evolve-dogfood-gate --layer all` 全緑（1a ✓ / ingest ✓ / Layer2 4項目 ✓ / Layer3 74 pass）
> 
> **切り出した残課題**
...
PR #503 の通し評価ゲート（`bin/evolve-dogfood-gate --layer 2`）実機1周で検出。
...
- `bin/evolve-dogfood-gate --layer 2` の observability_contract チェックが緑
...
- `write_pending_marker` docstring: 「emit が **dry-run でも書く**」「SessionStart の drain リマインドと `evolve --drain` の pending ソース」

標準 evolve フローは `evolve --dry-run` 分析のみ（#484 と同じ前提）なので、このゲートにより **emit→drain の決定論 accept/reject 捕捉（ADR-041）が全死**する: marker が書かれない → `drain_pending` 空振り → SessionStart リマインドも沈黙。
...
`skills/evolve/scripts/evolve.py` の module-level `DATA_DIR` は `Path.home() / ".claude" / "evolve-anything"` ハードコードで、`CLAUDE_PLUGIN_DATA` 環境変数を読まない（`scripts/lib/` 配下のモジュールは env 優先で解決する — common.py 同型）。`MARKER_ROOT`（evolve_decisions.py:41）も意図的に home 固定（hook/tool 合意のため、こちらは設計）。
...
dogfood gate Layer 1b（非 dry-run store 差分 — 「書かれるべきものが書かれる」方向）は #496 Wave 0 で NotImplemented 枠のみ予約し「#484 修正後に実装」とした。#484 は PR #512 でマージ済み・実環境検証済み（`evolve --drain` で written=18、決定論3チャネル初出現）なので前提解除。

## 実装内容

隔離コピー（PR #515 の `copy_data_dir_to_tmp` + `CLAUDE_PLUGIN_DATA` 伝播）を流用し、コピー側に対して `evolve --drain` を実行 → store 差分で以下を assert:
...
<!-- evolve-introspect:runtime_error:discover:nonetype-object-is-not-subscriptable -->
...
- 提案:
  ...
  - (b) evolve 実行中の stderr を self_analysis.runtime_errors の入力に実際に配線する
...
- **SLUG/OUT 再導出ボイラープレート**: SKILL.md の各 Bash ブロックで python ワンライナーによる SLUG 再導出 + OUT パス組み立てを繰り返す。evolve 側は slug 解決済みなので、`evolve --print-out-path` のような1コマンドを用意すれば SKILL.md の手順が大幅に短縮でき、assistant の書き損ね面も縮む（#402 の drain 1コマンド化と同型の改善）
...
SKILL.md Step 9 の #476-4 補正は「最後に実行した `evolve-reflect --promote-weak` 出力の `corrections_human`（昇格後の最新値）で `corrections（human-confirmed のみ）{最新値}/10` を上書き表示」と指示している。しかし実測:

- `growth_report` の `0/10` = **docs-platform スコープ**（promote 前は実際に 0 件で正しい）
- `evolve-reflect --promote-weak` の `corrections_human: 41` = **全PJ合計**（correction_idioms.jsonl の confirmed 内訳: evolve-anything 30 / sys-bots 6 / docs-platform 5）
...
---
環境: docs-platform / evolve 2026-06-12 run / dry-run 分析 → 対話適用の標準フロー
...
**修正案**: has_fitness=true かつ structural_reason あり のケースで「documentation 関数は evolve-optimize / evolve-loop-orchestrator 実行時に使われる。evolve の日次ループでは skill 提案が出ない限り母集団は貯まらない」のような 1 行を next_action に添える。
...
---
環境: docs-platform / evolve 2026-06-12 run
...
## Before / After（evolve-anything 運用者の体験）
...
- [ ] `/evolve-anything:audit` を回す → レポートに「Skill Vulnerability」section が出て `import` 済みスキルの danger 件数が表示される
...
- **Before**: evolve-scorer の3軸並列 fan-out が単一採点に勝つかを計測する仕組みが無く、論文の主張を自 PJ で検証できない。`subagent-guard` は件数ゲートのみで「効くか」は見ていない。
...
- evolve-scorer（オーケストレーター + 3 subagent 並列採点）の fan-out が単一比で勝つかの計測経路が無い。

## 配線先（recurring ループ）

- `telemetry` または `chaos` fitness に fan-out 効果軸を追加 → `audit` が消費し evolve のたびに surface
- evolve-scorer の3軸並列結果 vs 単一採点の delta を telemetry に記録

## 採用後の確認方法

- [ ] `/evolve-anything:evolve` を回す → telemetry section に「fan-out advantage delta」が出て、3軸並列が単一採点に勝った率が表示される
...
## Before / After（evolve-anything を回す開発者の体験）
...
## スコープ判断（重要）
SkillAudit は 89 タスクを**能動的に再実行**して軌跡を取る。evolve-anything は受動観測（再実行しない）設計なので full 移植は実行ハーネス新設が必要でコスト大。
...
## 採用後の確認方法
- [ ] `/evolve-anything:audit` を回す → report に「paired diff / skill 有無の挙動デルタ」section が出る（別コマンド不要で出るのが正しい配線）
...
## 採用後の確認方法
- [ ] `/evolve-anything:evolve` を回す → 矛盾検出に対し Repair 候補（置換案）が proposable に出る
...
## 採用後の確認方法
- [ ] `/evolve-anything:evolve` を回す → observability セクションに「skill 脆弱性: N 件 / ✓ 該当なし」が surface（0 件でも1行残す, silence≠evaluated）
...
1. `bin/evolve-fleet recall` のヒット時に、ヒットした memory ファイルへ `reinforce_memory(path, reason="recall hit")` を呼ぶ（recall ヒットを access proxy にする → access_count 非取得問題を回避）
2. session-start の memory 注入 hook で、注入した memory に同様に reinforce
- 減衰側は evolve→prune が既に消費しているので追加配線不要

## 採用後の確認方法

- [ ] `bin/evolve-fleet recall "<よく使う語>"` を実行 → 対象 memory の frontmatter `last_reinforced_at` が更新され `importance_score` が上がる（現状は変化しないはず）
...
- [ ] `/evolve-anything:evolve` を回す → report に「記憶操作 read/use/write/maintain」section が出る
...
- [ ] `/evolve-anything:evolve --dry-run` → skill_extractor 候補に「再実行検証: pass/fail」が付与され fail 候補が triage に渡らないことを確認
...
<!-- evolve-introspect:glossary-jargon-universal-stoplist -->
...
`cd /tmp && grep ... /tools/evolve-anything/scripts/evolve.py` や `/tmp/rl_evolve_out.json` を読むコマンド＝**evolve-anything 開発作業**だった（amamo の作業ではない）。cwd ベース帰属では「技術的に正しい」が、アプリPJのテレメトリに開発ノイズが載る。
...
<!-- evolve-introspect:discover-examples-truncate-and-crosspj-noise -->
...
<!-- evolve-introspect:auto-memory-rule-citation-reblock-loop -->
...
- `evolve-reflect --promote-weak` 出力 `corrections_human = 81`（**全PJ集計**）
...
## 提案
CLI(`evolve-reflect --promote-weak`) 出力を `corrections_human_allpj` のように **scope 明示リネーム**し、per-PJ 値と機械的に区別できるようにする。

<!-- evolve-introspect:corrections-human-scope-rename -->
...
<!-- evolve-introspect:bootstrap-bulk-theme-cluster-ux -->
...
<!-- evolve-introspect:fitness-evolution-output-contract-simplify -->
...
<!-- evolve-introspect:improvement:consistency_usage_suitability:evolve-skill -->
<!-- evolve-introspect:improvement:consistency_usage_suitability:discover -->
<!-- evolve-introspect:improvement:consistency_usage_suitability:feedback -->
<!-- evolve-introspect:improvement:consistency_usage_suitability:backfill -->
<!-- evolve-introspect:improvement:consistency_usage_suitability:audit -->
<!-- evolve-introspect:improvement:consistency_usage_suitability:pitfall-curate -->
<!-- evolve-introspect:improvement:consistency_usage_suitability:agent-brushup -->
<!-- evolve-introspect:improvement:consistency_usage_suitability:prune -->
<!-- evolve-introspect:improvement:consistency_usage_suitability:release-notes-review -->
<!-- evolve-introspect:improvement:consistency_usage_suitability:import -->
<!-- evolve-introspect:improvement:consistency_usage_suitability:spec-keeper -->
...
<!-- evolve-introspect:runtime_warning:constitutional_cache:constitutional-cache-stale-miss-audit-step-refresh-cache -->
...
- observability:「当PJ未昇格 35 件（うち**未読 18 件**）。未読分は `/evolve-anything:evolve` の
...
- 決定論チャネル 未読 M 件 → `/evolve-anything:reflect --promote-weak` で昇格
...
分母実測: evolve-anything=30, docs-platform=5, sys-bots=6  ← session 分母は十分だが
...
## 採用後の確認方法
- [ ] `/evolve-anything:evolve` を回す → レポートに「held-out / replay / 退行」セクションが出て、提案ごとに多視点ラベルが付く
...
## 採用後の確認方法
- [ ] `/evolve-anything:reflect` を回す → 提案根拠に閾値未満の無関係 correction が出ない（関連度スコア付きで表示される）
...
`~/.claude/evolve-anything/subagents.jsonl`、セッション `fe3653bc`（#567/#568/#569 follow-up を処理したセッション。実体は worker567 / worker568 + α）:
...
`/tech-eval` で実装した multiview_eval（#564, v1.103.0）を実PJ2つ（evolve-anything / docs-platform）で動作確認したところ、**実データでは必ず「✓ 評価したが該当視点なし」しか出ない**繋ぎ目バグを発見。
...
- `outcome_attribution` キー（`attribute_outcomes` = 起動時のスキル名）: プラグイン修飾形 `evolve-anything:cleanup` / `evolve-anything:spec-keeper` / `evolve-anything:docs-refresh`

→ 同一スキルが居るのにプレフィックスの有無で **交差が空集合**。実測（evolve-anything）で `target_skills ∩ attribution = 空`。chaos は設計上 None、negative_transfer も 0 件なので、outcome 由来3視点（過学習/コスト増/再利用可能）が**構造的に発火不能**。
...
audit observability builder → `collect_observability` → evolve が `result[observability]` に格納。`/evolve-anything:evolve` / `/evolve-anything:audit` を回せば surface。

## 採用後の確認

- [ ] 実PJ（evolve-anything）で `build_multiview_eval_section` を回し、`evolve-anything:cleanup` 等が bare `cleanup` に join され outcome 由来ラベルが付く（または中立判定が根拠付きで出る）
...
`/tech-eval` で実装した relevance_gate（#565 FinAcumen, v1.103.0）を evolve-anything の実 weak_signals 287件で動作確認。機構は正常（採点・kept/suppressed 分離・理由付与・降順ソート 全て機能）だが、**実文脈では kept=0 / suppressed=287 に倒れる**（自由文文脈の jaccard が max ~0.25・中央値 0.0 で閾値 0.5 に到達しない）。「関連経験を残し無関係を抑制」が目的なのに**全件抑制の no-op** になっている。
...
`bin/evolve-reflect --show-weak-signals --context <文脈>`。reflect 運用で過去経験を提案根拠に出す経路。

## 採用後の確認

- [ ] `bin/evolve-reflect --show-weak-signals --context <実文脈>` で部分一致（jaccard ~0.25）の関連経験が kept に出る（旧 0.5 では全 suppress）
...
## 提案
次のいずれか:
- 過去未読分を拾う明示導線を surface する（`/evolve-anything:reflect --promote-weak` への誘導を
...
<!-- evolve-introspect:evolve-weak-signals-promotable-guidance-has-no-entry-point-for-backlog -->
...
<!-- evolve-introspect:evolve-calibration-drift-shows-accumulation-count-when-structurally-unscorable -->
...
<!-- evolve-introspect:evolve-rule-violation-observed-has-no-enforce-hook-promotion-path -->
...
## 提案
PJスコープ時は producer 側で global 配列を **件数のみ**（とポインタ `bin/evolve-fleet status`）に削る。
...
<!-- evolve-introspect:evolve-prune-global-candidates-stored-as-full-array-in-result -->
...
<!-- evolve-introspect:evolve-usage-suppress-no-guaranteed-reevaluation-on-window-recovery -->
...
<!-- evolve-introspect:evolve-skill-procedure-dryrun-write-policy-not-centralized -->
...
<!-- evolve-introspect:bootstrap-theme-label-cjk-ngram-noise -->
...
<!-- evolve-introspect:confirmable-idiom-full-message-never-recurs -->
...
<!-- evolve-introspect:glossary-jargon-fp-generic-terms -->
...
<!-- evolve-introspect:outcome-slug-worktree-name-pollution -->
...
> 
> ### 補足（誤検知だったもの）
> ファイル名に "evolve" を含む `eval-sets/evolve.json` は **skill のトリガー eval セット**（`{"query": "/evolve-anything:evolve", "should_trigger": true}`）であり、PJ slug 混入ではない。同様に `evolve-state.json` / `skill-evolve-*.json` は feature ファイルで無関係。
> 
...
> 
> - write 側正規化（observe usage-registry / correction_detect corrections / promote reflect 昇格 の `project_path`）+ 集計側 `_pj_of` 統一 + 3ストアのバックフィル CLI（`bin/evolve-fleet migrate-pj-slug`）。
> - これは `.claude/worktrees/` 配下の worktree のみ write 時に直る。
> 
> **残課題は #602 に切り出しました**:
> 1. **sibling ディレクトリ worktree**（`evolve-anything-wt/issue-593` 等）は `pj_slug_fast` の構造的限界で write 時に親repoへ畳めない（hot-path で subprocess 禁止 → `resolve_pj_slug` 不可）。SessionStart 1回 resolve→cache 案。
...
<!-- evolve-introspect:promotion-readiness-pj-count-ambiguous-label -->
...
<!-- evolve-introspect:recommended-artifacts-no-decline-suppression -->
...
- SGCD のもう一つの新規性「off-trajectory 状態の合成（スキルなしで数ステップ走らせて現実的な軌道外状態に到達 → スキル誘導で完遂）」は evolve-anything の transcript 採掘ベース設計とは機構が異なる。**まずは failure-rollout マイニング + 失敗の罠軸の追加までをスコープとし、off-trajectory 合成は対象外**とする。
...
## 採用後の確認方法
- [ ] `/evolve-anything:evolve` を回す → discover の skill 候補に `outcome: failure` 由来の候補と「失敗の罠」軸フィールドが現れる
...
- RODS の「進捗報酬分散をゼロコスト境界検出器に再利用し、境界サンプルの構造に合う新変種をスキル整合リサンプリングで合成、方策と共進化する動的リプレイバッファ」のうち、**online data synthesis / co-evolving replay buffer は GRPO 勾配ベース RL の機構であり evolve-anything（LLM 1パスパッチ + regression gate、勾配学習なし）には乗らない**。
...
## 採用後の確認方法
- [ ] `/evolve-anything:audit` を回す → outcome_attribution のターゲットランキングに分散スコア列が出て、高分散スキルが上位に来る
...
- `tools/evolve-anything-wt/issue-593` や figma の `fable5` のような **sibling ディレクトリ worktree**（`.claude/worktrees/` 配下でない）はマーカーが無く、basename が幻PJ slug として記録され続ける。
...
- 現状 `bin/evolve-fleet migrate-pj-slug` は corrections / subagents / sessions.db の3ストアのみ。
...
- [ ] `bin/evolve-fleet migrate-pj-slug` が7ストアを dry-run 既定・冪等で正規化
...
`bin/evolve-dogfood-gate` の **Layer 3（SKILL.md コードブロック抽出実行）** が `skills/report-feedback/SKILL.md` の3ブロックで `existence_only` 赤を出す。pre-push hook（light）が毎 push で非ブロッキング警告を出し続けている。
...
## 参考
- 検出: `bin/evolve-dogfood-gate --layer light`（または `--layer all`）の Layer 3
...
<!-- evolve-introspect:reflect-count-degraded-sentinel-vs-none -->
...
<!-- evolve-introspect:introspect-dedup-ignores-closed-regressions -->

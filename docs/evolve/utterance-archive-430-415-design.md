...
[cold path] batch ingest（audit/evolve 実行時 + rl-fleet コマンド）
...
`bin/rl-fleet recall` の検索対象に utterances.db を追加（既存 keyword 決定論検索の流儀）。
...
- 実機 1 PJ E2E を pytest に必須化: rl-anything PJ の実 transcript で ingest を完走させ、

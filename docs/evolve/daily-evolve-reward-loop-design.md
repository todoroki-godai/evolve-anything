...
実測（2026-06-12, `~/.claude/rl-anything/`）:
...
> 「毎日 `/rl-anything:evolve` を叩くだけで、環境が自然に進化していく」
...
```
 ...
 (313, llm_judge)      │                                                                              │
          ...
          │   y → rl-reflect --promote-weak <signal_keys> + ★#2 idiom を human-confirmed 化            │
          │   Skip/Other → 確認なしで完走（分岐は §2.1 エッジケース）                                  │
          │ Step Report: result の growth_report を末尾表示                                            │
          └────────────────────────────────────────────────────────────────────────────────────────┘
                                                       │ apply 後
                                                       ▼
                              rl-evolve --drain（既存 ADR-041 経路に相乗り or 独立 promote）
```
...
- 「はい」を選んだ group の `signal_keys` をまとめて
  `rl-reflect --promote-weak <key1,key2,...>` で昇格（既存 promote_signals に相乗り）。
...
| ③ | **idiom 単位の取り消し** | `rl-reflect --revoke-idiom <idiom_key>`: 該当 idiom を `confirmed=False` + `revoked_at` に戻し、その idiom_key 由来の `promoted_by="idiom_dict"` corrections レコードを `invalidated=True` に原子的 rewrite（`_rewrite_promoted` と同型）。`count_human_corrections` は invalidated を除外 → フェーズ進捗が正しく巻き戻る。weak_signals 側の `promoted=True` は維持（再提示しない） |
...
116  figma-to-code      24  atlas-breeaders    13  sys-bots        6  docs-platform
 ...
 47  rl-anything        16  daily-ai-github-trending   9  aws-cost-guardian
```

**ハイブリッド方式（決定済み・論点3）**:
- **アクティブ PJ（rl-anything 47 件・figma-to-code 116 件など上位）**: 初回 bootstrap モードで
...
**本設計での変更**: builder の文言に **「未昇格 N 件 → /rl-anything:evolve の今日の修正確認で昇格可能」**
...
3. **bootstrap の方式** → **ハイブリッド**（§機能#3）。アクティブ PJ（rl-anything 47 件・

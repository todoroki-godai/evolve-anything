...
**evolve.py は `from evolve import run_evolve / _resolve_pj_slug / main` で外部（test 21 本・`bin/evolve`）から
...
4. **`bin/evolve-dogfood-gate --layer all`** — dry-run SHA256 不変 / report invariants / SKILL.md コードブロック実行。
...
1. **最も独立・最も低リスク** — ファイルを `evolve/__init__.py` に移すだけで振る舞いを 1 行も変えない。
   re-export が正しければ既存 21 test と `bin/evolve` がそのまま通る（壊れたら即 ImportError）。
...
| dry-run 書込ゲートの段違い破壊 | 🟡中 | post-batch 群（weak_signals/idiom 等）は `dry_run` を最下層 write まで貫通する設計。抽出時に `ctx.dry_run` の伝播を `bin/evolve-dogfood-gate` の dry-run 不変で確認 |

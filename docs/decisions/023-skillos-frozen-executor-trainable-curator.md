...
evolve-anything は Claude Code（executor）を凍結した状態で、スキル/ルール層（curator）のみを
...
- **SkillRepo**: Markdown + YAML frontmatter の skill 定義集（evolve-anything の `.claude/skills/` と同形式）
- **Curator の訓練**: GRPO（DeepSeek-R1 系のグループ相対 advantage）で policy 学習

## evolve-anything との対応関係

| SkillOS | evolve-anything | 評価 |
|---------|-------------|------|
| Frozen executor (Qwen3/Gemini) | Claude Code（凍結） | 完全同型 |
| Trainable curator π_𝒮 | evolve-anything plugin（進化対象） | 完全同型 |
| SkillRepo (MD + YAML) | `.claude/skills/` (MD + YAML) | 完全同型 |
| 3 操作 (insert/update/delete) | `skill_triage` 5択 (CREATE/UPDATE/SPLIT/MERGE/OK) | evolve-anything 優位 |
| r^comp 圧縮ペナルティ | telemetry fitness 軸（本 ADR 実装）| evolve-anything が採用 |
| r^fc valid call 率 | telemetry fitness 軸（本 ADR 実装）| evolve-anything が採用 |
| GRPO policy 学習 | LLM 1-pass + regression gate | SkillOS 優位（研究規模）|
| Safety / rollback | `scripts/lib/regression_gate.py` | evolve-anything 優位 |

## Decision

evolve-anything の「Claude Code を frozen executor、plugin 層を trainable curator として分離する」
...
2. evolve-anything の LLM 1-pass + regression gate が現在の規模に適切

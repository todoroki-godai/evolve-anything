...
- `promoted_records`（雛形）は `promote_signals` の**戻り値**と `--promote-weak` の
  **stdout JSON への追加キー**であり、`corrections.jsonl` 等の永続化ストアにフィールドを足さない。
  evolve phases の永続化された result キー・新規ストア・新規 weak_signal channel のいずれでもない。
- ただし **#379 新設凍結の解除は過去に人間裁定ではなく頭が単独で決めた前例が無い**。
  前例の #587 では「2026-08-30 のユーザー裁定」「#379の新設凍結をこの1件だけ解除する
  （ユーザー裁定 2026-09-01）」と明記されており（`gh issue view 587` 本文）、
  凍結の対象範囲に関する解釈（stdout の一時キーが凍結対象に該当するか）を頭が自分で確定して
  進めることはしない。**本設計は「対象外と考える理由」までを書き、可否の最終確定はユーザー裁定を
  仰ぐ（未確定のまま実装 PR を出さない）**。

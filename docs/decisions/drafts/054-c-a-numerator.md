...
- 起点は daily runner が安定稼働した 2026-08-11 以降。最初の確定週は **W34 = 2026-08-17（月）〜 08-23（日）**、
  ...
  **【2026-08-18 実測で訂正・#508 M2/M10】上記の「W33 は100%に届かず対象外」は W33 進行中に書かれた予測であり、
  実測ではなかった。#508 の dry-run 実測では W33（08-10〜08-16）は 629/629 = 100%・`measured=True` に到達している
  （ただし M8: production 実行時点で `measured=True` の週は W33 の1件のみで `best_run_length=0` のため、
  この時点でも系列表示ゲート k=4 週連続は未成立）

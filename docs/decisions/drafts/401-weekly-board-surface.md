...
4. **runner 側の想定外例外（`build_weekly_board` の外側）は fail-open でキーを書かない**（その日の SessionStart は沈黙し、翌日の実行で再計算される）。コードレビュー巡3で、例外時に `{measured: False}` を書く追加が本番経路（`main()` を `now` なしで呼ぶ）で `AttributeError` になり queue 全体を書けなくする欠陥が見つかった。巡数上限（3巡）に達したため、ユーザー裁定（2026-09-11）でその追加を取り除き、巡2で確認済みの形に戻した。発生件数: 0件（`measure()` の中の失敗は `build_weekly_board` が捕まえるため、ここへ届くのは想定外の例外だけ）。例外時にも health を残す改善は別 issue に切り出す（巡数3を引き継ぐ）

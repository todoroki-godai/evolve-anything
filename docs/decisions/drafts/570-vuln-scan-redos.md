...
**`hits` 要素型の境界**: 戻り値契約では `hits` が list であることまでを検証し、各要素が
`pattern_id` / `category` / `line` 属性を持つ型かまでは検証しない。要素は同一モジュール内の
`ContaminationHit` 生成物であり、ここまで防御を広げると内部型向けの検証機構を増やすためである。
不正要素なら `AttributeError` が外へ伝播するが、write 前で中断してキューも消化されないため、
安全性を保ったまま次回再試行になる。

**カウンタの扱い**: 検査不能（import 失敗・呼び出し例外・戻り値契約破損）を内訳として示す
`guard_unavailable` を追加する。既存利用者の集計値を変えない後方互換のため、これらは従来どおり
`contaminated` にも合算する。したがって `guard_unavailable` は `contaminated` のうち実際の
汚染検出ではなく検査不成立で止めた件数を表す。
...
- `guard_unavailable` は検査不能件数を分離表示する一方、後方互換のため `contaminated` にも
  合算される。そのため両値を足すと二重計上になる
- **[Should]2（検査不能だった候補の扱い）への判断（採用: 永久消化）**: 検査不能時
  ...
  **一時的な障害（例: memory_guard モジュールの一時的な import エラー、依存パッケージの
  ...
  発生しない）。**`EVOLVE_MEMORY_GUARD=warn` への降格は検査不能の経路には効かない。**
  この点は本設計では未解決のまま残す

...
- **原理的に検出不能な唯一の残存ケース（§8.1手順2・§15検証(1)で確認する）**: フェーズ1
  ...
  **これはフェーズ1という「最初の1回の書込み」自体の成否に還元される問題であり、
  ...
  構造的な事実として記録する
- **2フェーズを `--skip` に適用しなかった理由**: `--skip` は§0③により柱2の集計対象外
  （監査証跡のみ）であり、①②の完成条件に直接効かない。2フェーズ化のコスト
  （イベント種別・fold ロジックの複雑化）に見合わないと判断した。将来 `--skip` も
  集計対象にする場合は同型の2フェーズ化を追加できる
...
- **retention は `"permanent"` のまま**（実装1巡に含めない・前巡の頭の裁定を継承）。
  **retention 根拠の訂正（Q6 Should）**: 第5版は「イベント数は corrections 件数を
  超えない」としていたが、本設計は1回の `--apply` 成功で最大2行（`correction_apply_attempted`
  + `correction_applied`）を書き、失敗した試行や `--skip` のイベントも累積するため、
  **イベント行数は基底レコード件数の部分集合ではなく、反映操作の試行回数に比例して
  増える**（誤りだった記述を訂正）。正確な成長モデルは実測していないため、
  再検討の引き金は「イベント数が基底件数の10倍を超えた時点」等の相対閾値ではなく
  **絶対件数の閾値**（`reflect_apply_events.jsonl` が10,000行を超えた時点、を暫定値と
  する——`corrections.jsonl` の現在規模247行の40倍程度で、個人利用規模の `--apply`
  実行頻度からは当面到達しないと見込むが未実測）を再検討の引き金とする
...
| **7（新規）** | **`--skip` を2フェーズ化しない判断（§16）でよいか。将来 `--skip` も集計対象にする計画があるなら今のうちに2フェーズ化しておく選択肢もある** | **要判断（タイムアウト既定: 判断が付かない場合は現行案どおり2フェーズ化しない。`--skip`は現時点で柱2の対象外のため実害は無い）** |
| **8（新規）** | **`reflect_apply_events.jsonl` の再検討閾値を絶対件数10,000行とした暫定値（§16）でよいか** | **要判断（タイムアウト既定: 判断が付かない場合は現行の10,000行を暫定値のまま次巡以降へ持ち越す）** |

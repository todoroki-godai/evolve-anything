...
- Status: **Draft・保留（2026-08-25 縮小。実装着手しない）**
  - 保留理由: Codex 移行後（2026-08-23〜25）の rollout ログ 126 セッションを実測したところ、
    122 件が `codex exec` の非対話レビュー実行で、**実ユーザー発話は1セッション1回**（ユーザー発話
    3回以上は 8 件のみ）。会話が成立していないため、本 ADR が抽出対象とする「ユーザーが AI を

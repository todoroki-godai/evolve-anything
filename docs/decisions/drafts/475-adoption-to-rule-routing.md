...
| 外す反映先 | 理由 |
|---|---|
| **skill** | 既に `skill_evolve` レーンが独自の y/n と採用履歴（`optimize_history`）を持つ。ここから直接書くと同じ採用が2レーンに分かれて記録され、**`bin/evolve-revert --list` に出ない skill 変更**が生まれる（revert 迂回）。skill に書きたい指摘は既存レーンへ回す |
| **hook** | P15 のとおり revert 経路が存在しない。**戻せない反映**を朝の30秒の中で作らせるのは柱4（1コマンドで戻せる）と正面から衝突する。hook 提案は #467 の未接続13種側で扱う |

初期スコープ外の2種は Other（自由記述）で意思表示できるが、**その場では書かない**。
既存レーン（skill_evolve / hook 提案）へ回した旨を表示して終わる。

`AskUserQuestion` は1問4択が上限（U1）。**採る方式**: 4 反映先のうち**上位2つを先頭に置き**、
続けて「いまは反映しない」「いいえ」を置く。残り2反映先へは **Other（自由記述）** で到達する。
...
| 3 | **いまは反映しない（記録だけ残す）** | **AI の振る舞いは変わりません。** 記録は棚卸しの件数と他PJでの優先提示にしか使われず、**90日経つと自動削除されます**（§12 判断2 で「消さない」を選べば残ります）。あとで反映したくなったら `/evolve-anything:reflect` でまとめて出せます（朝の確認には再度出しません）。 |
...
memory / pitfall が上位に来た場合はその反映先が 1 または 2 の位置に入り、押し出された rule は
Other へ回る（Other から初期スコープ4種すべてに到達できることが前提・U2）。

**選択肢3・4は常設（並べ替えの対象にしない）**: 「いまは反映しない」と「いいえ」は
指摘の内容にかかわらず必ず同じ位置に出す（tacchi Must-5）。却下手段が消えると不要な指摘が
毎朝出続け、承認疲れが1日5件の上限（P8）を超える。§12 の判断1 はこれで**決定**とし、
ユーザー判断からは外した。
...
| skill（初期スコープ外・Other） | — | **この場では書かない**。`skill_evolve` レーンへ回した旨を表示して終わる | — | 既存レーンの y/n |
| hook（初期スコープ外・Other） | — | **この場では書かない**。戻せないため（P15）朝の設問からは作らせない | — | — |
...
| suppression | `discover/suppression.py:196` | **変えない**。用途が別（discover 提案の抑制）で、広げる利得が無い |

**帰結**: 「いまは反映しない」は「朝からは消えるが reflect には残る」状態になる。
§4.3 の文言 3 は「あとで反映したくなったら、朝の確認でもう一度出せます」ではなく
**「あとで `/evolve-anything:reflect` を実行したときにまとめて出せます」**が正しい。文言を差し替える。
...
| 項目 | 内容 |
|---|---|
| 実装場所 | `reflect.py:470` の `update_reflect_status` に確認を内蔵する（呼び出し側に任せない。**新規 CLI サブコマンドは作らない**＝`feedback_evolve_single_entrypoint`） |
| 確認方法 | 反映先ファイルを読み、起草した行（§4.3 の `draft_line`）の**正規化後の完全一致**が存在するか。正規化は前後空白と行頭 `- ` の有無のみ（LLM 不要・決定論） |
| 不一致のとき | `applied` を書かない。`promoted` のまま残し、`{"status":"apply_unverified", "target": …}` を返す。**黙って成功にしない** |
...
| F11 | skill / hook を Other で選んだユーザーが「回した」と言われて終わり、実際には既存レーンに何も渡らない | **気づけない（初期スコープの限界）** | 初期スコープ外なので本設計は表示するだけ。既存レーンへの受け渡しを配線するのは別 issue（§11-U7） |
...
| U2 | Other（自由記述）から memory / pitfall へ確実に到達できるか | **未実測**（U1 と同根） | 到達できないなら初期スコープ4種を1問に収める前提が崩れ、2段階設問の再検討が必要になる |
...
| U7 | Other で skill / hook を選んだ意思を既存レーンへ実際に受け渡せるか | **未実測（初期スコープ外）** | 受け渡しが無ければ F11 のとおり「回した」が空約束になる。初期スコープでは**表示のみ**と明示し、配線は別 issue に切る |
| U8 | 起草1行の事前提示（Must-6）が朝の30秒を壊さないか | **未実測**。1問あたりの読む量が増える | 提示が長すぎると「朝の30秒」が崩れる。実装時に `draft_line` を**1行・全角60字以内**に制限し、超えたら省略記号で切る |
...
`~/.claude/rules/design-review-gate.md` に従い、**実装着手前に1巡入れる**。
固定項目「この設計が依拠する前提のうち実測で裏取りされていないものを列挙せよ。
測れば分かるのに測っていない前提は [Must] とせよ」を prompt に含める。

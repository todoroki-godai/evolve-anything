...
correction は当PJ 75 件中 **3 件**だった（8 件のうち 4 件は `readable-report` の実体が
どこにも無く解決不能、1 件は critical 行 0 で本番の出力に現れないまま消えている）。
上表の「解決 46」が旧規則の値である点は変わらない。**判定器が取りこぼしているかどうかは
その分解でも判定できない**（3 件はいずれも違反なしと判定されたが、見逃しでも同じ 0 になる）。

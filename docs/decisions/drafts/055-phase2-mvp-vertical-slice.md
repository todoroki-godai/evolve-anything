...
- Status: **Draft・凍結（2026-08-25。実装着手しない）**
  - **凍結理由**: 親 ADR-055 が 2026-08-25 に保留へ縮小され、Phase 1〜3 の実行指示が
    すべて凍結された（`055-codex-rollout-ingest.md:3-7`）。本追補はその Phase 体系の
    上に「Phase 1.5」を挿入する設計であり、**親が凍結された以上、本追補も単独では
    着手できない**。
  - **以降の本文は凍結前（2026-08-24 版）の記述であり、正典ではない**。Phase 1.5 の
    手順・Decision（D1.5-1〜）・計測の実行契約を含め、**本文にある実行指示はすべて
    凍結する**。
  - **解凍条件**: 親 ADR-055 の再測条件（#534 のコメント 2026-08-25 が正典）が満たされ、
    親が保留から Go へ移った時点。その時点で本追補は**設計から見直す**（凍結中に
    前提が動いている可能性があるため、そのまま着手しない）。
  - **保存目的**: 実測ベースの配線図（file:line 付き）と、Phase 番号の整合方法・
    Non-goals の切り分けは、親が解凍された際に再利用できる。破棄せず凍結して残す。
  - 追加のレビュー巡は発注しない（`review-round-cap.md`。凍結は挙動を変えないため）。
- Date: 2026-08-24

...
evolve-anything v1.7.0 の observe hooks は 7 イベントを処理していたが、Claude Code v2.1.69〜v2.1.78 で追加された以下の機能が未活用だった:
...
- **evolve-scorer の maxTurns/disallowedTools**: `maxTurns: 15` でコスト上限設定。`disallowedTools: [Edit, Write, Bash]` で採点エージェントのコード変更を禁止。Agent は除外（サブエージェント起動に必要）
- **DATA_DIR の CLAUDE_PLUGIN_DATA フォールバック**: `${CLAUDE_PLUGIN_DATA}` 優先、未設定時は `~/.claude/evolve-anything/` にフォールバック。既存データの一括マイグレーションは行わない（段階的移行）
...
- **disallowedTools に Agent を含める**: evolve-scorer 自身がサブエージェントを起動するため不可
...
- evolve-scorer の maxTurns/disallowedTools によりコスト制御と安全性が向上

...
```python
DATA_DIR = Path(env) if (env := os.environ.get("CLAUDE_PLUGIN_DATA")) else Path.home()/".claude"/"rl-anything"
```

- **hook 実行時**: CC が `CLAUDE_PLUGIN_DATA` を設定 → `~/.claude/plugins/data/rl-anything-rl-anything/`（plugin-data dir）に書く
- **standalone tool/skill 実行時**: env 未設定 → fallback `~/.claude/rl-anything/` に解決
...
3. それも無ければ install レイアウト `~/.claude/plugins/data/<*rl-anything*>` を
...
- resolver は plugin-data の install レイアウト命名（`<marketplace>-rl-anything`）に
  依存する。プラグイン名 `rl-anything` を含む dir を mtime 降順で 1 つ選ぶ。

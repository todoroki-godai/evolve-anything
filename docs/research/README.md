...
1. **自動化パイプラインは evolve-anything として実装済み** — 7 hooks + 14 スキルで Observe → Discover → Enrich → Optimize → Reorganize → Prune → Reflect → Report の9フェーズを自律実行
2. **本ディレクトリは evolve-anything 設計時の調査資料**。実装の正確な仕様は [README.md](../../README.md) を参照
3. **PostToolUseフック + Stopフック自動リフレクション** の構想は hooks/ として実現済み
4. claude-reflect は evolve-anything が独自実装したため採用せず（参考資料として保持）

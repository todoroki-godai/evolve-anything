...
**柱1の戦果ボード（#685）**: 調整に使っていない確認用セットの TP を正規表現または AI 判定候補の
どちらかで拾った率を主行とし、調整に使った a0 は上振れしうる参考値（到達の根拠にしない）として表示する。正規表現単独の `L1捕捉率` も併記する。
AI 陽性は朝の y/n 前の候補であり、本番保存件数ではない。評価 ID/本文/ラベルの
一対一突合と判定版・モデル別名・バッチ設定の来歴が揃わない場合、主行は測定不能とし、a0 を繰り上げない。
目標値は #567 を参照（確認用セットの主行に適用することは #685 のコメントに記録）。詳細は
...
修正在庫 `correction_backlog >= 1` の PJ も列挙する。在庫は `material_count` に加算しない。
tracked 外は `untracked_with_material`、dead/phantom は処理不能 advisory として透明化し、
表と JSON の各 entry に `correction_backlog` を出す。
...
スキル25個・共通ロジック27パッケージ（`scripts/lib/` 配下）・bin/ コマンド26個・適応度関数8個組み込み（`default`/`skill_quality`/`coherence`/`telemetry`/`constitutional`/`chaos`/`environment`/`plugin`）・userConfig 21項目で構成。個別モジュール・パッケージの詳細は [spec/architecture.md](spec/architecture.md#observe--報酬信号レイヤー詳細) を参照。
...
**採用の巻き戻し（#402 / [ADR-053](docs/decisions/053-revert-cli-design.md)）**: 体験4「skill 採用は1コマンドで戻せる」の実体。accept 記録に変更前本文を持たせ（PR-1）、`bin/evolve-revert <entry_id>`（既定 dry-run・`--apply` で実書込）で戻す（PR-2）。revert は削除でなく **revert イベントの追記**で表現し、判断母集団は `fold_effective` を単一ソースとする effective view（`load_effective_history`）で読む — 生ログを読む raw reader は AST allowlist 3件に固定した（`raw_history_gate`）。**適用範囲は skill 採用のみ**（remediation の rules/hooks は ADR-041 の意図的スコープ外）。**現データでは戻せる entry は 0 件**（既存 accept はすべて記録拡張前＝`pre_extension`。実効は次に採用する分から）。詳細は [spec/components-core.md](spec/components-core.md)（`evolve_revert` / `raw_history_gate` / `optimize_history_store` の effective view）を参照。

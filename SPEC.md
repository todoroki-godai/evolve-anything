...
Last updated: 2026-04-28 by /spec-keeper update
...
スキル25個、共通ロジック43モジュール（fleet 含む）、bin/ コマンド15個（`rl-fleet` 追加）、適応度関数8個組み込み（`default` / `skill_quality` / `coherence` / `telemetry` / `constitutional` / `chaos` / `environment` / `plugin`）。
...
- 2026-04-28: **CC v2.1.121 対応** — `hooks/tool_duration.py` 新設（Bash 1秒超の実行時間を `tool_durations.jsonl` に記録 / `duration_ms` CC v2.1.119+ 対応）。`skills/evolve/SKILL.md` + `skills/rl-loop-orchestrator/SKILL.md` に `${CLAUDE_EFFORT}` 条件分岐を追加。`CLAUDE.md` Quick Start に `claude plugin prune` 追記。グローバル `~/.claude/agents/doc-writer.md` に `maxTurns: 25` 追加
- 2026-04-27: **breakthrough スキル追加 (v1.40.0)** — 「惜しいがブレイクスルーしない」問題を5フェーズで解決。行き詰まりタイプ（A-E）を診断し Tutor-Student / MAgICoRe / Devil's Advocate 等の戦略を自動選択して Agent 起動まで一貫実行。戦略カタログ (`references/strategies.md`) + エージェントプロンプトテンプレート (`references/agent-templates.md`) 同梱
...
- 2026-04-22: **cleanup: tmp prefix userConfig 化** — `manifest.userConfig` に `cleanup_tmp_prefixes` (string, カンマ区切り, default `"rl-anything-"`) を追加。`scripts/lib/cleanup_scanner.py::parse_prefix_config` で list 化（trim / 空要素除去 / 重複排除 / `None` 許容）。SKILL.md 実行時に `[cleanup] tmp scan scope: [...]` を宣言表示（closes #71）

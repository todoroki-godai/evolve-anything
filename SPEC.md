...
Last updated: 2026-05-06 by /spec-keeper update
...
Observe hooks (16個, LLMコストゼロ) → テレメトリ JSONL → evolve/discover/reflect/audit → remediation → 自動改善。
スキル25個、共通ロジック47モジュール（fleet / session_store / score_noise 等含む）、bin/ コマンド18個（`rl-gain` / `rl-score-noise` / `rl-prompt-compare` 追加）、適応度関数8個組み込み（`default` / `skill_quality` / `coherence` / `telemetry` / `constitutional` / `chaos` / `environment` / `plugin`）。
...
- 2026-05-01: **rl-prompt-compare: Evaluator プロンプト A/B 比較** — `bin/rl-prompt-compare` + `scripts/lib/scorer_prompts.py` 新設。候補プロンプトを参照スキルで N 回計測し mean drift・σ 比較・recommended を出力。`CLAUDE_PLUGIN_DATA/scorer_prompts/{axis}.txt` でオーバーライド可能。テスト 11件
- 2026-04-30: **sessions: DuckDB SoR 完全移行** — `scripts/lib/session_store.py` 新設（Repository パターン）。`telemetry_query.query_sessions` を sessions テーブル直参照に切り替え。`discover.py` / `evolve.py` / `backfill.py` の sessions.jsonl 直読みを session_store 経由に統一。sessions.jsonl 廃止（legacy バックアップ残置）

...
Last updated: 2026-06-05 by /spec-keeper update (recovery) — ADR-038 SubagentStop additionalContext + ADR-037 Phase 1d 完了 (#323-#325) を反映
...
- 2026-06-05: **feat(claude-p 全廃): ADR-037 Phase 1d-ii — remediation fixers の2相化 (#324)** — `fix_line_limit_violation`（非 rule 圧縮・rule 分離）/ `fix_split_candidate`（スキル分割提案）の3つの claude -p サイトを撤廃。全て決定論フォールバック（proposable 降格 / 決定論 proposal_text）で完走し、LLM 品質回復は新規 `remediation/fixers_llm.py` の emit/ingest 6関数（`llm_broker` 活用、emit は IO-free・LLM-free）が担う。evolve SKILL.md Step 5.5.1 に2相品質回復を追記。回帰ゲート `CONVERTED_MODULES` に3モジュール追加、`KNOWN_REMAINING` を2件（score_noise / auto_memory_runner）に削減。[ADR-037]
- 2026-06-04: **feat(claude-p 全廃): ADR-037 Phase 1d-i — reflect 検出系の2相化 (#323)** — `semantic_detector`（`semantic_analyze` 削除、`validate_corrections`/`detect_contradictions` を LLM-free 化）と `critical_instruction_extractor`（`_call_llm_judge` 削除、`rephrase_to_calm`/`detect_instruction_violation` を LLM-free 化）の4つの claude -p 経路を撤廃。2相 API（`emit_validation_requests`/`ingest_validation_results` ほか）を追加、呼び出し元（reflect.py / discover/runner.py）はシグネチャ温存で無改修。LLM 品質は reflect SKILL.md Step 5.5 が回復。`KNOWN_REMAINING` を網羅化し「全 claude -p caller は CONVERTED か KNOWN のどちらかに必ず載る」不変条件を明文化。TDD（semantic 36 + critical/e2e 29 緑）。[ADR-037]
- 2026-06-04: **feat(claude-p 全廃): ADR-037 Phase 1c — evolve 系 scoring/proposal の2相化 (#322)** — `skill_evolve` の最後の claude -p 経路2つを撤廃。①`llm_scoring`: judgment_complexity 採点を `_score_judgment_complexity_static`（決定論フォールバック）+ 2相に分離、cache に `judgment_source`（static/llm）flag を持たせ legacy は次回 refresh で1回だけ昇格（収束）。②`proposal`: テンプレカスタマイズを2相化し fence 除去 + diff budget gate + 必須セクション再チェックの三段防御でフォールバック。`compute_llm_scores`/`evolve_skill_proposal` は LLM-free 化。回帰ゲート `CONVERTED_MODULES` に両者追加。TDD（full sweep 3876 passed）。[ADR-037]
- 2026-06-04: **feat(claude-p 全廃): ADR-037 Phase 1b — constitutional/principles の2相化** — scoring 軸 `principles._extract_via_llm` / `constitutional._evaluate_layer`（claude -p haiku）を撤廃し `llm_broker` 3相へ分離。順依存があるため SKILL は **principles round → constitutional round** の順で回す（`principles_missing` で順序違反を検知）。`environment` は cache-only read 化。audit SKILL に Step 3.5（2相）追加。回帰ゲート `CONVERTED_MODULES` に両者追加。テスト subprocess mock 全廃（full sweep 3855 passed）。[ADR-037]

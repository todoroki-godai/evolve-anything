...
Last updated: 2026-05-19 by /spec-keeper update (recovery×2)
...
Observe hooks (18個 registered, LLMコストゼロ) → テレメトリ JSONL → evolve/discover/reflect/audit → remediation → 自動改善。
ユーザー向けスキル19個 + 内部スキル（reorganize / enrich deprecated）、共通ロジック14パッケージ・136モジュール（scripts/lib/ 配下、audit/discover/fleet/rl_common 等パッケージ化済み）、bin/ コマンド18個（`rl-gain` / `rl-score-noise` / `rl-prompt-compare` 含む）、適応度関数8個組み込み（`default` / `skill_quality` / `coherence` / `telemetry` / `constitutional` / `chaos` / `environment` / `plugin`）。

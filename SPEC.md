...
Last updated: 2026-06-03 by /spec-keeper update (CLAUDE.md Skills パーサ記法拡張 + 解決状態3分類 #295, ADR-032)
...
全32件（最新: [ADR-032](docs/decisions/032-claude-md-skills-parser-coverage-and-resolution-tristate.md) CLAUDE.md Skills パーサに太字ラベル+バッククォートコマンド記法を追加し、CLAUDE.md 解決状態を3分類（不在=検出走らせる/在るが trigger抽出0=skip+surface/抽出成功=除外有効）して環境解決失敗を confident な誤検出にしない — 寛容パーサの際限ない拡張は [ADR-027] で否定済みだが CLAUDE.md は書式強制できない外部入力かつ過剰捕捉が安全側なので足切り拡張、#295）。SkillOS（Frozen Executor + Trainable Curator）/ MemOS（4層メモリ結晶化）対応設計の詳細は [spec/key-design-decisions.md](spec/key-design-decisions.md) を参照。カテゴリ別要約は [spec/architecture.md](spec/architecture.md#key-design-decisions-カテゴリ別サマリ)、ADR 原文は [docs/decisions/](docs/decisions/) を参照。

...
Last updated: 2026-05-27 by /spec-keeper update (recovery)
...
- 2026-05-27: **feat(memory): AlphaSignal-inspired importance scoring (#233)** — `scripts/lib/memory_temporal.py` に `importance_score` / `last_reinforced_at` フィールドと `compute_importance_score()` / `reinforce_memory()` / `write_importance_score()` を追加（アトミック書き込み）。`auto_memory_runner` でエントリ作成時に自動採点。`audit/memory.py` に低重要度候補セクション（importance_score ≤ 0.3）を追加
- 2026-05-27: **feat(skill-evolve): Co-ReAct-inspired rubric checkpoint visualization (#231)** — `scripts/lib/skill_evolve/rubric.py` 新設。`rubric_checkpoint(phase, proposal_dict)` が propose/apply フェーズのチェック項目を stdout に可視化（pitfalls / pre_flight / trigger / diff_budget / reason_refs）。`count_diff_lines` を public API 化し proposal dict に `diff_lines` フィールド追加
- 2026-05-27: **feat(evolve): RPG 物語ナレーション + fix(audit/bloat): .archive/ 除外 + custom スキル判定 v1.69.0 (#234)** — `world_context.py` 追加（PJ 固有世界観を JSON 永続化、LLM で初回生成しデフォルトフォールバック付き）。`skills/evolve/SKILL.md` に Step 0.5（世界観ロード）+ Discover/Remediation/Prune/Report の4段階ナレーション指示を追加。`.archive/` 配下の SKILL.md を `find_artifacts` rglob から除外。`skills_count` / audit レポートを custom スキルのみで判定。`world-context.json` に `total_evolve_count` / `last_evolve_date` / `current_level` / `previous_level` を自動更新

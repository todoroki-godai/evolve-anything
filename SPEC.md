...
Last updated: 2026-05-29 by /spec-keeper update (observability contract / ADR-028)
...
全29件（最新: [ADR-028](docs/decisions/028-observability-contract-audit-evolve.md) observability は markdown 選択読みでなく audit↔evolve の構造化 contract で surface する — 単一ソース `_OBSERVABILITY_BUILDERS` を markdown/構造化の両経路が消費）。SkillOS（Frozen Executor + Trainable Curator）/ MemOS（4層メモリ結晶化）対応設計の詳細は [spec/key-design-decisions.md](spec/key-design-decisions.md) を参照。カテゴリ別要約は [spec/architecture.md](spec/architecture.md#key-design-decisions-カテゴリ別サマリ)、ADR 原文は [docs/decisions/](docs/decisions/) を参照。

## Recent Changes

直近の変更概要（完全な履歴は [CHANGELOG.md](CHANGELOG.md)）:
- 2026-05-29: **feat(evolve): observability contract — 必ず surface すべき行を構造化フィールドに昇格 (#278)** — #272 で audit markdown に `✓` 行を出したが、evolve は 217KB markdown を `phases.audit.report` に丸ごと格納し assistant が名前付きフェーズを選択読みする運用のため、中盤に埋もれた observability 行が surface されなかった（docs-platform の ev-v6 が v1.78.0 でも ✓ 行をログに出さず表面化＝silence≠evaluated が観測性 fix 自身の配線で再発）。`audit/observability.py` を新設し `_OBSERVABILITY_BUILDERS`（glossary_drift / unmanaged_pitfalls の単一ソース）+ `collect_observability` を定義、`report.py`(markdown) を単一ソース消費に統一、`run_evolve` が `result["observability"]` へ格納し SKILL.md Step 3.8 で必ず surface（MUST）。将来 observability 項目は builder 1行登録で両経路に自動伝播（モグラ叩き回避）。単一ソース drift ガードを含む contract テスト7件。実 PJ docs-platform で E2E 確認。[ADR-028]
- 2026-05-29: **feat(evolve): 用語集 seed 作成トリガーを #278 observability contract に統合 (#275)** — #273 の Step 7.7（CONTEXT.md 無し時の LLM seed 提案）が docs-platform 実 evolve（ev-v6）で発火しなかった（phase に裏打ちされない散文ステップが dry-run の谷間で消えた、install≠enforcement の最深レイヤ）。本 PR 初版は独立 `glossary_seed` phase に格上げしたが、並行 merge の #278 が surface 行を `_OBSERVABILITY_BUILDERS` 単一ソースに集約したため seed 判定もそこへ統合（surface パターンを1本化）。`build_glossary_drift_section` を拡張し CONTEXT.md 不在 ∧ undefined jargon ≥ `SEED_MIN_CANDIDATES` のとき「用語集未作成」行を emit → markdown と `result.observability.glossary_drift` の両経路へ自動 surface。独立 phase / `check_glossary_seed()` は撤去、SKILL.md Step 7.7 を observability 消費型へ。回帰テスト（builder seed 分岐 + contract surfacing + jargon 薄時の沈黙）。seed writer（`write_context_seed`, #273）は既存流用
- 2026-05-29: **fix(fitness): `constitutional`/`chaos` の `_load_sibling` がパッケージ化 coherence を silent skip (#277)** — #143 で `coherence/` パッケージ化した際 `_load_sibling` の追従が `environment.py` だけに入り、`constitutional.py`/`chaos.py` は `{name}.py` 固定のまま → `coherence.py` 不在で `FileNotFoundError` → constitutional スコアが coherence 軸を欠いたまま silent skip していた。`environment.py` の package 対応版を両ファイルへ移植。回帰テスト3件 + docs-platform 実 evolve で skip 消失を確認。install≠enforcement の silent skip 型
- 2026-05-29: **feat(audit): 未登録 pitfalls.md を Unmanaged Pitfalls advisory で可視化 (#271)** — #265/#266 の pitfall 自動強制はオプトインで、育っている pitfalls.md があっても `enable` 登録まで hook 無反応＝enable 漏れがどこにも surface しなかった。`pitfall_registry.unmanaged_candidates`（discover − managed の純粋集合差・stdlib のみ）+ `parse.count_entries`（正準パーサ再利用の liveness 指標）+ `audit/sections.build_unmanaged_pitfalls_section`（未登録∧エントリ3+件のみ提示、書きかけ・空はノイズ抑制）を新設し `report.generate_report` に glossary と同形で配線。検出は audit・enable 実行は pitfall-curate に責務分離。実リポジトリでドッグフード（誤検出ゼロ）。同時に optimizer integration テストの mock 先を `optimize_core.subprocess` へ修正（リファクタ追従、4 failed→9 passed）
- 2026-05-29: **feat(spec-keeper): CONTEXT.md 用語集 + drift 検出 (#268)** — tech-eval `mattpocock/skills` 由来。PJ 固有 jargon を 1 語で decode する `CONTEXT.md`（Ubiquitous Language）を新設。鮮度は `glossary_drift.py`（決定論・LLM 非依存）が検出: 構造 drift（malformed/duplicate/missing_first_seen）を gate、SoT 未登録 jargon は advisory（gate しない＝オオカミ少年化回避）。spec-keeper update フローに加え **audit にも `build_glossary_drift_section` を配線**（evolve は audit を消費するため evolve のたびに用語集の鮮度が surface ＝手動コマンド依存を排除、install≠enforcement の是正）。さらに **evolve に Step 7.7「用語集ブートストラップ」**を追加 — CONTEXT.md 無し∧未登録 jargon ≥ `SEED_MIN_CANDIDATES` のとき AskUserQuestion で LLM seed 生成を提案（creation→detection を一本化）。生成行は初出列に `⚠UNVERIFIED` を付け drift gate でなく `unverified_terms` advisory で人間確認を促す（誤り毒 + 検出器自滅の回避）。writer `write_context_seed` は決定論・非破壊。実コーパスでドッグフード済み

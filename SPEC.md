...
全23件。カテゴリ別要約は [spec/architecture.md#key-design-decisions-カテゴリ別サマリ](spec/architecture.md#key-design-decisions-カテゴリ別サマリ)、原文は [docs/decisions/](docs/decisions/) を参照。
...
- **warn 超 5件の対応** — `agent_quality.py` (531行) / `workflow_checkpoint.py` (462行) / `skill_triage.py` (458行) / `layer_diagnose.py` (433行) / `audit/orchestrator.py` (420行) が warn 閾値 (500行) 超。hard (800行) 到達時に fleet パターンで分割。`reflect_utils.py` (534行) は scripts/ ルートに置かれており scripts/lib/ への移動予定

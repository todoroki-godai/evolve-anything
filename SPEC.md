...
Last updated: 2026-05-20 by /spec-keeper update (recovery)
...
ユーザー向けスキル20個 + 内部スキル（reorganize / enrich deprecated）、共通ロジック14パッケージ・154モジュール（scripts/lib/ 配下、audit/discover/fleet/rl_common 等パッケージ化済み。`pipeline_eval.py` / `skill_importer.py` 追加）、bin/ コマンド18個（`rl-gain` / `rl-score-notify` / `rl-prompt-compare` 含む）、適応度関数8個組み込み（`default` / `skill_quality` / `coherence` / `telemetry` / `constitutional` / `chaos` / `environment` / `plugin`）。evolve パイプラインに意図確認層（`intention_check` in `regression_gate.py`）を組み込み、high-risk 変更を BLOCK。
...
- 2026-05-19: **feat(telemetry): r^comp / r^fc を telemetry fitness 5 軸に追加 (closes #67)** — `rate_completion` (r^comp) / `function_call_validity` (r^fc) を `scripts/lib/telemetry_query/` と `scripts/rl/fitness/telemetry.py` に統合

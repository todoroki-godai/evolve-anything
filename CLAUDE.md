...
| `memory_conflict` | 同一 PJ 内の非両立記憶ペア（同一 specific key を肯定/否定で言及）を決定論検出する audit advisory section（記憶健全性の第3軸・memory_temporal/contagion と直交, #83）。極性は SOV/SVO 語順非対称（日本語否定=forward 窓 / 英語否定=backward 窓）で判定し「A ではなく B」型を誤検出しない。保守側（specific key 限定・ambiguous drop で FP 回避）・floor3・superseded 除外・memory dir は `resolve_cc_memory_dir`・新ストア無しの読み取り専用 advisory | `audit/memory_conflict.py` + `audit/sections_conflict.py` |

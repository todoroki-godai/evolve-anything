...
**これは到達目標であって現状説明ではない**。3 と 4 の一部は未達（下記の実測注記）。この節を新機能の
採否判定に使うときは、目標文でなく各項目の実測注記の側を基準にする（#376「数字が嘘をつかない」を
自分の看板文言にも適用する・2026-08-13 tacchi/codex レビュー）。
...
3. **週1**: 戦果ボードで事実を棚卸しする（採用件数・一発成功率・戻せる採用の一覧）。
   **効果の因果判定（「手直し回数の減少」＝(a) / 「採用した改善が効いたか」＝(b)）は分母が揃った
   ...
   計測可能になって初めて発火する。**実測（2026-08-13）: (a) は hook 捕捉 recall 4.5%・後継の
   `correction_semantic` レーンの recall は未計測、(b) は有効な accept が0件（10件は #376 で全て
   無効化）＝現時点でどちらも数字を出す資格がない**（ADR-054 §5・§7.2）
4. **信頼**: 表示する数字が嘘をつかない（#376）/ 適用は必ず人間の y/n（無人適用しない）/
   skill 採用は1コマンドで戻せる（**適用範囲: evolve drain 経由の新規採用のみ**。optimize.py 経路と
   evolve-loop 経路は revert 対象外＝ADR-054 Phase D PR2/PR3 を凍結した。実測で採用実績が
   それぞれ 0件 / 実質1件のため。使われ始めたら解凍する）
...
| `evolve_revert` | 採用した skill diff を戻す apply engine。3分岐（normal/冪等/conflict）で conflict は上書きせず中止、CLI は既定 dry-run・`--apply` のみ実書込。`--list` は entry_id 一覧の read-only 導線（ADR-054 D2） | `evolve_revert/` + `evolve_revert_listing.py` + `bin/evolve-revert` + `evolve_revert_cli.py`（ADR-053/ADR-054） |

...
| `subagent_traces` | subagent 内部軌跡ストア（#38）— subagents.jsonl の transcript から内部 tool error/やり直し回数を per-agent_type で advisory 表示し、親 error_count しか見ない outcome 帰属の盲点を塞ぐ。新ストア `subagent_traces.jsonl`（active・batch writer）。低品質 ⚠ 発火＝一発成功率<0.5 or 平均 tool error≥5（#76）。増分 ingest は `evolve --drain` 境界に配線＝run_evolve 死蔵による全PJ停滞を根治（#135） | `subagent_traces/` + `audit/sections_subagent_traces.py` |

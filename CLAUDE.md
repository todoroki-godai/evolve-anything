...
| `fleet_pr` | 承認済み evolve 提案の worktree→commit→push→PR 化。commit は `--commit-path` allowlist のみ stage・push はアカウント検証ゲート付き。適用とマージは常に人間（#82, #268） | `fleet/pr.py` + `fleet/cli_pr.py` |

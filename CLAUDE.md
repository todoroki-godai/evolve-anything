...
| `agent-brushup` | エージェント定義の品質診断・改善提案・upstream 監視・model exact-ID pin 検出（#449）+ addyosmani skill anatomy 欠落節の根拠付き改善提案（#63）+ tools 宣言と実付与の乖離検出（`memory:` の Write/Edit 自動付与・#130） | `agent_quality.py` |
...
| `subagent_traces` | subagent 内部軌跡ストア（#38）— subagents.jsonl の transcript から内部 tool error/やり直し回数を per-agent_type で advisory 表示し、親 error_count しか見ない outcome 帰属の盲点を塞ぐ。新ストア `subagent_traces.jsonl`（active・batch writer）。低品質 ⚠ 発火＝一発成功率<0.5 or 平均 tool error≥5（#76）。増分 ingest は `evolve --drain` 境界に配線＝run_evolve 死蔵による全PJ停滞を根治（#135）。`write_trace(record, data_dir=)` で read/write の隔離先を対称化＝data_dir 指定時は例外口 store_write_raw で隔離先へ書き「read は隔離・write は常に本番」の非対称契約と誤書込事故を根治（#140） | `subagent_traces/` + `audit/sections_subagent_traces.py` |
...
| `artifacts_hygiene` | artifact 衛生4検出器（#124 グローバル CLAUDE.md 空/未存在 / #125 SKILL.md 欠落 dir / #126 バックアップ残置 / #129 skill 名跨 scope 重複・symlink wrapper 除外）を #115 advisory 共通枠で observability に surface。2026-07-03 PC 環境手動監査の検出器ギャップ起票分（決定論・LLM 非依存） | `audit/sections_artifacts.py` |

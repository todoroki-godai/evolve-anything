...
| `userConfig` | CC v2.1.83 manifest.userConfig で trigger 閾値（auto_trigger/interval/cooldown 等）と cleanup スキル prefix（`cleanup_tmp_prefixes`）・slow command 閾値（`slow_threshold_ms`）・subagent 警告閾値（`subagent_warning_threshold`）を含む 10 項目をプラグイン有効化時に設定可能 |

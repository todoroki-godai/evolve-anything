...
| `weak_signals` | 暗黙修正シグナルの決定論検出（直後手編集 / permission deny / 言い直し / Esc 中断）→ weak_signals.jsonl レーン。corrections 直入れせず reflect 確認後に昇格。言い直し閾値は実コーパス dry-run で決定（jaccard 0.8）。45日 TTL で期限切れを expired マークし昇格候補から除外（#442）+ observability に evolve 昇格誘導文言（#444）+ **TTL 失効を read 時 age 計算で write 非依存化（`is_effectively_expired`・#89）— 標準フロー（dry-run→drain）は `mark_expired` を通らずフラグ未書込のため、`read_unpromoted`/`material_count` が `detected_at` から age を導出して 45日超を落とす。実機 dogfood で queue material 膨張の潜在 seam として発見**[#432] | `weak_signals/` |
...
| `review_channels` | y/n 確認に出す weak チャネルの単一ソース（#99）— `REVIEW_CHANNELS`（content-rich）+ `signal_text`（channel 別 actionable 代表テキスト）を bootstrap_backlog/daily_review/promote が共有。promote の message も決定論チャネルで channel 名の空 correction を防ぐ + `grouping_keywords` で review group 化キーを表示テキストと分離し permission_deny を拒否コマンド単位で group 化（固定 head による 1-group collapse を解消・#99 F1 review 指摘）+ **F1 follow（実 dogfood）**: grouping_keywords が tool_input_summary 全体を token 化し作業ディレクトリ path segment が grouping を支配 → 別コマンド（force push vs checkout）が共通パスで collapse する over-merge を `_strip_path_words`（'/' を含む語を除外）で解消（実 5 件 3→4 group）+ signal_text 単語境界切り詰め（途中切れ解消） | `correction_semantic/review_channels.py` |
...
| `fleet_queue` | 学習素材ベースの evolve 待ち列挙 `evolve-fleet queue`（#79 Phase 1a）— material_count = weak 未処理 + 前回 evolve 以降の新規 corr が `--threshold`（既定5・env `EVOLVE_QUEUE_THRESHOLD`）以上の PJ を決定論・ゼロ LLM で列挙。`select_evolve_queue` 純関数。新ストア `evolve-queue-state.jsonl`（per-PJ `last_evolve_at`・store_registry active・`writer_locus="batch"`・store_write barrier 経由・`evolve --drain` apply 境界で書込）が「前回 evolve 以降」を PJ 別に測る（既存グローバル `evolve-state.json` を補完）。corrections の `project_path` は `project_name_from_dir` で weak_signals `pj_slug` と名前空間統一。`--json` は Phase 1b #80 契約。dogfood 修正（#79）: tracked_projects の実 dir 不在 PJ（rename 済 `rl-anything` 等）は queue から除外し `skipped_dead` で透明化（footer `（skipped N dead: …）`・silent truncation 禁止）/ 各 entry に `project_path` を付与し利用側の親 dir ハードコード推測を撲滅（`~/games` 配下等の取りこぼし解消）/ weak・corr 集計を `pj_slug_aliases_for`（read 層別名 SoT）経由にし旧 slug レコードを現 slug に畳む。**#85**: `activity_since.sessions` の常時0ハードコードを `collectors.aggregate_sessions_by_project`（session_store union read・distinct session_id・`aggregate_subagents_by_project` 対称）で実値配線。**#86**: queue 母集団（tracked）と material 母集団（weak/corr 全 slug）の不一致是正 — `collect_untracked_materials` が material 持ち untracked PJ を CC discovery 実 dir gate（phantom/temp slug 除外）付きで `untracked_with_material` に advisory surface（auto-track せず人間が `evolve-fleet discover` 承認）、footer 母数を `M tracked (config)` に明示。 **#87/#88**: rename-but-live PJ（tracked=旧 dead path・store=旧 slug・discovery=新 live dir）の material が `skipped_dead` に無音で飲み込まれ evolve-anything 自身が queue から消えた seam を是正 — dead 判定で canonical slug が discovery live dir に解決すれば redirect し waiting に乗せる（#87①）/ 真の dead にも material 数を添えて透明化（#87②）/ `fold_activity_counts` で activity_since を alias fold し weak/corr と namespace 統一（#87③・実 155 sessions が旧 slug で 0 落ちを根治）/ 閾値以上 material の未解決 untracked slug を `collect_phantom_materials`→`skipped_phantom`+footer `（skipped N phantom: …）` で透明化（#88・skipped_dead と対称）。実機 dogfood で発見・config の dead tracked path 修正と併用。 **#91/#92**: `project_path` 欠落 corr（実機9件・backfill8/hook1）が `_correction_slug` 空落ちでどの母数にも入らず queue 完全不可視だったのを `count_unattributed_corrections`→`unattributed_corrections` キー + footer `（PJ 未帰属 corrections: N 件 — source 内訳）` で透明化（#91・「無音で落とさない」最後の穴）/ cold-start（never）の REASON を `weak=N + corr=M（初回・全件）` に出し分け `new corr` との矛盾視認を解消（#92・表示のみ）。 **A/①（dogfood 2026-06-26・tacchi 採点）**: cold-start の material は増分でなく**全履歴 backlog の全件**計上ゆえ material_count は velocity でなく累積量＝ランキングは累積順。純 cold-start（全待ち PJ が never）時のみ footer に `（初回表示: corr は全履歴を計上＝累積量順で velocity ではありません…）` を surface し誤ランキング（stale backlog を上位と誤認）を防止（`_append_coldstart_notice`・一部 drained は混在ノイズゆえ非表示）。併せて REASON の内部 plumbing 語 `未 drain`→業務語 `初回・全件` に置換。B（below-threshold 件数 surface）/ C（calibration ゲートの daily 監視）は tacchi 採点で見送り（B=`tracked−waiting` で導出可能・非 actionable / C=検出器はデータ生産者 /evolve と同居ゆえ cron 化は未変化データ再スキャン）。②（未帰属 hook=1）は実測で 2026-05-19 の歴史的残渣・以降ゼロと判明し live writer リーク否定＝観察のみ。 **#94/②（2026-06-26 dogfood）**: bootstrap 破棄済み weak を queue が TTL まで二重計上した seam を read 時導出で除外（marker 設置時刻前 detected を `_exclude_bootstrap_consumed` で落とす・footer `（bootstrap 消化済みを待ちから除外…）`・#94）+ WEAK 列の意味（未処理のみ＝promoted/TTL/bootstrap 消化を除いた実残数）を footer 明示（`_append_weak_semantics`・②・#96 PR 同梱）。**#113**: material の weak を content-rich channel（`REVIEW_CHANNELS`）に限定し、content-poor（esc/手編集＝昇格不能）は footer/`--json` `weak_content_poor` で透明化。 | `fleet/queue.py` + `fleet/queue_state.py` + `fleet/cli.py` + `fleet/collectors.py` + `fleet/formatters.py` |

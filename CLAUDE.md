...
| `judge_audit` | LLM judge の false-pass 欠陥注入監査（The Blind Curator arXiv 2607.07436・#188）。既知の欠陥 fixture 6件（正解=違反ありと決定論的に既知）を judge の実プロンプト/パーサ（`constitutional._build_eval_prompt`/`_parse_layer_response` 再利用）に流し、合格(judge 自身の rubric 基準 score>=0.8)と誤判定する割合を opt-in CLI ハーネス（dry-run 既定・llm-batch-guard 準拠）で計測。judge の false-pass はスキル退役（negative_transfer rollback）を無音で無効化するため事前計測する。floor=min(5, fixture総数)（`effective_min_judged`。fixture数が floor 未満でも全件判定済みなら必ず率が出る不変式）・false-pass 率20%超で ⚠ を advisory surface（verbosity #75 と同型の分離パターン） | `scripts/lib/judge_audit/` + `audit/sections_judge_audit.py` |

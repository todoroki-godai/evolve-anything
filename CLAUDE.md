...
| `review_channels` | y/n 確認に出す weak チャネルの単一ソース（#99）— `REVIEW_CHANNELS`（content-rich）+ `signal_text`（channel 別 actionable 代表テキスト）を bootstrap_backlog/daily_review/promote が共有。promote の message も決定論チャネルで channel 名の空 correction を防ぐ | `correction_semantic/review_channels.py` |

...
```bash
bin/evolve-audit --growth  # 戦果ボード = 柱1（調整に不使用の確認用セットの正規表現＋AI候補の合計捕捉率。a0 は参考・旧L1内訳）・柱2（採用件数）・柱3（指摘率の測定可否・除外内訳）
...
| フィードバック | reflect, report-feedback | reflect=修正パターン検出 → corrections.jsonl → CLAUDE.md/rules に反映。**反映先は rule に限らない — skill / hook / pitfall / memory も柱2の対象**（種別ごとの到達状況と接続計画は #467 が正典。ここに現状の可否を書かない）。report-feedback=evolve/audit レポートを LLM メタレビュー → evolve-anything 自身への改善 issue を todoroki-godai/evolve-anything に半自動起票（決定論 evolve_introspect が拾えない「読んで気づく」改善が対象。旧 feedback スキルの後継） |
...
| pitfall 運用 | pitfall-curate | 任意PJの pitfalls.md を育てる PJ非依存ツール。類似 dedup / 普遍性分類（universal/project/instance + 汎用度1-5）/ 三段階開示の配布版(Top-N)生成 / 記録↔分類↔配布の同期ゲート。判断は agent、決定論処理は `skills/pitfall-curate/scripts/pitfall_curate.py`。`pitfall_manager`（自己進化専用）とは別物 |
...
- `reflect_apply_events`: reflect `--apply`/`--skip`/`--revoke` が専用境界で追記し、取り消しは target ごとの CAS 遷移として畳む。正準 DATA_DIR への generic/raw 直書きは常に reject

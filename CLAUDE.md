...
| `evolve_decision_ids` | 提案 identity（`_proposal_id`）と判断イベント identity（`_decision_event_id`）を隣り合わせに置く純関数 module。両者の取り違えが #279→#286→#290 の3連発の根因なので定義を1箇所に集約（#287） | `evolve_decision_ids.py` | `evolve_decisions.py` |

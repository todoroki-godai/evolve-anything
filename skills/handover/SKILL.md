...
### Issue 化判断（コンパクト出力後に自動評価）

コンパクト形式を出力したら、会話コンテキストを振り返り「GitHub Issue として残す価値があるか」を判断する。
Issue はチームや将来の自分が経緯を追えるように残すもの。単純な作業ログは Issue にしなくてよい。

**Issue を提案する条件（1つでも該当すれば提案）**:
- 設計上の重要な判断があった（なぜその実装を選んだか、廃案が何かが価値を持つ）
- 未解決のブロッカーや複数の観察中タスクがある
- 複数人が関わる可能性のある決定・方針変更が含まれる
- 「観察中」セクションに2件以上ある

**Issue を提案しない条件（いずれも当てはまれば不要）**:
- 定型的な実装・PR 作成・マージのみ
- 次にやることが1件で単純な続き作業
- 判断の経緯が git log や PR description に既に記録されている

判断結果に応じて：
- **提案する場合**: コンパクト出力の直後に1行だけ添える：
  `→ 設計判断が含まれています。Issue も残しますか？（yes で作成）`
- **提案しない場合**: 何も追加せず終了する

ユーザーが「yes」「はい」「お願い」などと返答したら Step 3c を実行する。
「no」「いい」「不要」や無反応なら終了する。

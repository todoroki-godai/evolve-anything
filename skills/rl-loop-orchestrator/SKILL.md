...
ベースライン取得 → バリエーション生成 → 評価 → 人間確認のサイクルを回し、スキル/ルールを自律的に改善する。
...
[Step 1] ベースライン取得: 現在のスキルを rl-scorer で採点
    ↓
[Step 2] バリエーション生成: genetic-prompt-optimizer で3つのバリエーションを生成
    ↓
...
- 1ループの API コスト目安: sonnet x 3回（バリエーション生成）+ haiku x 3回（評価）

...
- **行数チェック**: グローバル `~/.claude/rules/code-quality.md` の「rules は統合テーマごとに10行以内」に従い、
  見出し行（`# タイトル`）と frontmatter を除いた本文が 10 行以内か。**基準はこの1箇所が単一ソース**で、
  ここに数値を再掲しない（過去に存在しない `rules-style.md` の「3 行以内」を参照しており、実基準 10 行と
  食い違ったまま全 rule を誤判定しうる状態だった）。PJ 側に独自の行数ルールがあればそちらが優先

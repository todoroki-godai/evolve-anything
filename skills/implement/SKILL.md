...
### Step 0.2: 既存実装との突合（このスキル内では必須）

**書き始める前に「同じものが既に無いか」を実物で確認する。** 計画に書かれていないことは
「存在しない」根拠にならない（計画自体が既存を見ずに書かれている場合がある）。

> **強制力の範囲**: これは `/implement` を通ったときの手順であり、**enforcement ではない**。
> 直接の Edit/Write や他スキル経由の実装はこの手順を迂回する（ADR-044 / ADR-045 の
> 「SKILL.md の MUST は enforcement ではない」に該当）。決定論的に強制するには
> PreToolUse(Edit|Write) hook で marker 不在の初回書込みを拒否する必要がある（#494）。

#### 探す

キーワードは計画から 2〜3 語。**表記ゆれ（英日・スネーク/キャメル・略語）ごとに個別に**
検索する。1回の grep で複数語を OR 検索しない（どの語が当たったか分からなくなる）。

```bash
# 1. 実装が既にあるか — 実装 root と拡張子を絞らない
grep -rnF "<keyword>" scripts/ hooks/ skills/ bin/ .claude-plugin/ docs/ \
  --exclude-dir=__pycache__ --exclude-dir=.git
# 2. 過去に実装・却下されていないか（打切りなし）
git log --oneline --grep "<keyword>"
grep -nF "<keyword>" CHANGELOG.md
# 3. 仕様・設計判断での位置づけ
grep -rnF "<keyword>" SPEC.md spec/ docs/decisions/ 2>/dev/null
```

**出力を件数で打ち切らない。** 多すぎて読めない場合は「該当なし」ではなく
**「判定不能」**として扱い、語を絞り直して再検索する。

#### 判定（上から順に当てはめ、最初に一致したものを採る）

| 状態 | 行動 |
|---|---|
| 1. 同じ機能が実装済み・稼働中 | **実装しない。** 何があるかを報告して指示を仰ぐ |
| 2. 部分的に実装済み・稼働中 | 新規に作らず**既存を拡張**する |
| 3. 同型の処理があり契約も適合 | **その部品へ寄せる**（再発明は同じ欠陥を N 回再生産する） |
| 4. 同型だが契約が合わない | 分離して作ってよい。**分けた理由を commit か ADR に残す** |
| 5. 実装済みだが未配線・deprecated・却下済み・複数の矛盾実装 | ADR と履歴を確認し、**質問してから決める**（ADR-030 / ADR-026） |
| 6. 他 PJ にありそうだが所在・契約を確認できない | **判定不能**。参照可否を確定させてから進む |
| 7. 上を尽くして何も無い | 「探した範囲と語」を1行残して Step 0.5 へ |

1〜3 は同時に成立しうる。その場合は**番号の小さい方**を採る（止まる側に倒す）。

無いことの確認を省くと、同型の実装と欠陥を独立に再生産する（#49 / #103-#105 / #159 / #160）。

**未実測の前提**: この手順を置くことで実際に再生産事故が減るかは測っていない。
起動率・実行率・検出率の before/after は #494 で扱う。

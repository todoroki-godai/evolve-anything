...
```bash
evolve-usage-log "audit"
evolve-audit "$(pwd)" --coherence-score
```
...
```bash
evolve-audit "$(pwd)" --telemetry-score
```
...
```bash
evolve-audit "$(pwd)" --constitutional-score
```

Constitutional Score: 各原則 × 各レイヤーの遵守度を LLM Judge で評価。Coherence Coverage < 0.5 の場合はスキップ。
[ADR-037] により `evolve-audit --constitutional-score` 本体は claude -p を呼ばず cache
...
```bash
evolve-audit "$(pwd)" --coherence-score --telemetry-score --constitutional-score
```
...
`evolve-audit` の出力に「未使用グローバルスキル」セクションが含まれる場合、
その内容をレポートに表示し、`/evolve-anything:prune` による整理を提案する。
...
1. 検証コンテキストを取得:
```bash
evolve-audit "$(pwd)" --memory-context
```
...
optimize / evolve-loop の実行履歴がある場合:

```bash
evolve-audit-aggregate
```
...
- audit パイプライン（`evolve-audit` 本体）は LLM を呼ばず既存 baselines を読むだけ。再スコアは
...
- Unmanaged Pitfalls（自動強制 未登録）→ 育っている（エントリ3+件）のに pitfall lint/commit-gate 未登録の `pitfalls.md` を提示。`/evolve-anything:pitfall-curate` での enable を誘導（install ≠ enforcement の可視化）

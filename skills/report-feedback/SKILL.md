...
effort: low
description: |
  evolve / audit のレポートを「上位の目」でメタレビューし、evolve-anything プラグイン
  自体の改善点・バグ・提案を抽出して todoroki-godai/evolve-anything に GitHub issue 起票する。
  他PJで /evolve-anything:evolve や /evolve-anything:audit を回した直後、その会話で
  「レポート見て改善点ある？」「evolve-anything のバグっぽい所は？」「これ issue にして」
  と言われたら必ずこのスキルを使う。決定論 self-analysis（evolve_introspect）が拾えない
  「人が読んで気づく」種類の改善（表示の分かりにくさ・提案の質・誤検知・UX）が対象。
  会話から直接 evolve-anything へフィードバック/バグ報告したいときも、このスキルで起票する
  （旧 feedback スキルの後継）。
  Trigger: report-feedback, レポート見て改善, evolve レポートレビュー, audit レポートレビュー,
  evolve-anything 改善点, evolve-anything バグ, メタレビュー, feedback, フィードバック, バグ報告,
  bug report, 機能提案, feature request, これ issue にして
---

# /evolve-anything:report-feedback — レポートからの改善フィードバック起票

evolve / audit のレポートを LLM がメタレビューし、**evolve-anything プラグイン自身**への
改善 issue を `todoroki-godai/evolve-anything` に半自動起票する。会話からの直接フィードバックにも対応する。

## 何を見るスキルか（最重要・取り違え防止）

evolve/audit のレポートは **対象環境（他PJのスキル/ルール）の改善** を出すもので、その**中身（結論）は
evolve-anything 自身の話ではない**。このスキルは中身を report してはいけない。見るのは **レポートの出来栄え・
挙動** — レポートを鏡にして **道具（evolve-anything）自身**を映す:
...
判定の物差し: 「これは evolve-anything のコード/挙動を直せば良くなるか？」が Yes なら対象。
...
| **会話経路**（旧 feedback 後継） | 会話中に evolve-anything のバグ/要望に気づいた | 直近の会話コンテキスト |
...
- **audit 経路**: `evolve-audit` は **stdout にレポートを出すだけ**（markdown/json 出力フラグも
...
```bash
ls -t ~/.claude/evolve-anything/evolve-results/*.json 2>/dev/null | head -1
```
...
レポート（または会話）を読み、evolve-anything **自身**への改善候補を抽出する。
...
open / closed **両方**の issue を取得し（`--state all`）、`filter_duplicates` で重複を除く（${CLAUDE_PLUGIN_ROOT} 経由）。
closed も取るのは、過去に直したはずの issue が再発（regression）したとき前歴へ backlink するため（#33）:
...
regressions は `unique` にも含まれる（＝新規起票対象）。前回 closed の #N と同一 root cause が
再発したものなので、Step 6 では `render_issue_body` でなく `render_regression_body(cand, N)` で
body 冒頭にバックリンクを差し込んで起票する（`N` は同 dedup_key の `regressions[i]["existing_number"]`）。
これで「一度直したはずが再発した＝不完全な修正だった」文脈がレビュアーに伝わる。
...
- PJ 名・ローカルパス・対象PJのスキル名/ルール本文・ビジネス固有名 → evolve-anything の挙動として一般化
...
Step 4 の `regressions` に同 dedup_key がある候補は、代わりに `render_regression_body(cand, N)`
（`N`=前回 closed の番号）で起票し、body 冒頭に前歴 backlink を入れる:
...
起票後、issue URL をユーザーに表示する。失敗時は `~/.claude/evolve-anything/feedback-drafts/` に
タイムスタンプ付きで保存してフォールバックする。

## プライバシー（MUST NOT — 起票先は public repo）

- **対象PJ固有の情報を含めない**: PJ 名・ローカルファイルパス・対象PJのスキル名やルール本文・
  ビジネス情報。報告対象は **evolve-anything の挙動**であって、それが動いた相手PJの中身ではない。
- SKILL.md の内容そのものを貼り付けない。
- 迷ったら「evolve-anything のどの挙動が問題か」に一般化して書く。

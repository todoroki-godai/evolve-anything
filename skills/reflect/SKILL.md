...
既存 memory ファイルに LLM 経由で書き換えを適用する場合、frontmatter の `update_count` を **+1** すること（存在しなければ `update_count: 1` を追加）。これは Issue #97 / arXiv:2605.12978 の「LLM 自己更新メモリの劣化」リスク (詳細: `docs/research/faulty-updated-memories.md`) に対する guard。

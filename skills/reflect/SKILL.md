...
audit の `memory_heavy_update` issue は `update_count >= 3` **かつ** 行数 >= 30 **かつ** memory が非健全（stale か superseded）の 3 条件で発火する (`scripts/lib/audit/issues.py`, `MEMORY_HEAVY_UPDATE_THRESHOLD`)。健全（非 stale・非 superseded）な高頻度更新メモリは `memory_capability` の use 軸で「活性（良）」として加点される対象なので heavy_update から除外する（#104: 同一 run で「健全」と「要対応」に二重分類する矛盾を防ぐ）。

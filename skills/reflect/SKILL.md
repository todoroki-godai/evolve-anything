...
# /evolve-anything:reflect — corrections の対話的レビューと反映
...
/evolve-anything:reflect --dry-run              # 分析のみ（書き込みなし）
/evolve-anything:reflect --view                 # pending 一覧表示
/evolve-anything:reflect --skip-all             # 全 pending をスキップ
...
```bash
evolve-usage-log "reflect"
evolve-reflect [オプション]
```
...
```bash
evolve-reflect --promote-episodic \
...
audit の `memory_heavy_update` issue は `update_count >= 10` **かつ** 行数 >= 80 の複合条件で発火する (`scripts/lib/audit/issues.py`, `MEMORY_HEAVY_UPDATE_THRESHOLD`)。update_count は `memory_capability` の use_read 軸で「活性（良）」として加点される指標なので、低い閾値で発火させると同一 run で「活性」と「要対応」に二重分類する矛盾を生む。閾値を引き上げ「churn + 肥大化」の劣化シグナルに純化することで、簡潔だが活発なメモリ（正常運用）を誤検知しない（#104）。この閾値は上記 Step 7.6 の `update_count >= 3` 再要約ガードとは別物（あちらは再要約による情報減損の warning）。

#### update_count リセット手順

`update_count >= 3` の memory を根本から書き直したい場合（情報減損を疑う場合）は以下を実行する:

1. **元 corrections.jsonl を参照** — `~/.claude/evolve-anything/corrections.jsonl` から該当 memory の元 corrections を `source_correction_ids` で特定し、生の修正内容を確認する
...
4. **audit で確認** — `/evolve-anything:audit` を実行し `memory_heavy_update` issue が解消されたことを確認する

...
audit の `memory_heavy_update` issue は `update_count >= 3` を gate にしつつ、**構造的肥大化**（行数 >= `MEMORY_HEAVY_UPDATE_LINE_THRESHOLD` または重複セクション >= `MEMORY_HEAVY_UPDATE_DUP_SECTION_THRESHOLD`）が伴う場合のみ flag する (`scripts/lib/audit/issues.py`)。`update_count` は memory_capability の use 軸では「活性=良」として加点される指標のため、更新回数単独では flag せず構造指標へ寄せている（#104）。この reflect 時の更新前 warning（`update_count >= 3`）とは別レーンであり、活発な健全メモリを audit が肥大化扱いしない。

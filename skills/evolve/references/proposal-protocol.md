# 提案詳細プロトコル（全 AskUserQuestion 共通）

SKILL.md の「提案詳細プロトコル」節から詳細・rationale を分離したもの。
**4 点提示（対象・根拠・変更内容・推奨）と 4 択上限は SKILL.md 側に MUST one-liner として残してある。**
ここはその背景と方式 A/B の手順詳細。

## なぜこのプロトコルが要るか

evolve が「やりますか？」と尋ねる前に、ユーザーが Yes/No を判断できる材料を提示するための共通ルール。
件数や閾値だけ出して承認を求めると、ユーザーは「何が・なぜ・どう変わるか」が分からず判断できない。
実際このプロトコルが無かったため、effort frontmatter 提案が「active スキル 10件」とだけ表示され、
どのスキルにどの effort が付くのか分からない、という問題が起きた。

## 4 点提示の詳細（AskUserQuestion を出す前に per-item 展開）

- **対象**: 具体名（`skill-name` / `path/to/file.py:42` / ルール名）。「N件」だけに丸めない
- **根拠**: なぜ検出したか。閾値・metric・evidence の**実値**を出す（例: `content_lines=62 < 80`, `confidence=0.90`, `ゼロ呼び出し・最終使用 45日前`）
- **変更内容**: 適用すると何がどう変わるか。可能なら before → after か diff の1行要約（例: `effort: (なし) → low`）
- **推奨**: どの選択肢を選ぶべきかと理由を1行（例: `推奨: はい — 3回検知・低リスク・1コマンドで戻せる`）。**「対象・根拠・変更内容」は材料であって答えではない** — 材料だけ並べて判断をユーザーへ丸投げしない。判断材料が足りず推せないときは `推奨なし: <理由>` と書く（**空欄・省略は MUST NOT**）

**多件数の扱い**: per-item 展開は**最大 10 件**まで。超過分は「他 M 件」と件数で示し、
「全件確認するには <コマンド or ファイルパス>」と誘導する。10 件以内なら全件展開する。

## AskUserQuestion の options 上限（4択制約）

`AskUserQuestion` の `options` は**最大 4 件**まで。
proposable 件数が 5 件以上の場合、1度の質問にまとめてはならない（MUST NOT）。
以下のいずれかの方式で対応する:

- **方式 A — 1 件ずつ順番に質問**: per-item で「はい / スキップ / 残り全てスキップ」の3択で進む（推奨）
- **方式 B — グループ分割**: 最大4件を1グループとし、グループごとに「一括承認 / 個別確認 / スキップ」の3択で進む
- **「その他 M 件」誘導**: 上位4件を個別提示し、5件目以降は「他 M 件（全件確認: <コマンド>）」へ誘導する

上記いずれの方式でも、4 点提示（対象・根拠・変更内容・推奨）は省略しない（MUST）。

## Python が detail を持っている前提

多くの提案は issue の `detail`（`skill_name` / `confidence` / `reason` 等）や
`generate_proposals(issues, project_root=Path(result["project_dir"]))`（[references/remediation.md](remediation.md) 参照。
`result` は Step 1 で Read した `$OUT` の JSON — `Path.cwd()` は単一 cwd から他 PJ の issue を
渡すバッチ経路（#400）で誤った PJ の代替パスを提案する）の `{proposal, rationale}` に
per-item の判断材料が既に入っている。
件数に丸めるのは表示側の問題なので、`detail` を読んで展開すれば追加の集計は不要。

このプロトコルは Step 2 / Step 5.5 / Step 7 / Step 7.5 など全提案ポイントに適用する。

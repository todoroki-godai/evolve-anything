...
`$PJ` は SKILL.md Step 1 で束縛済みの対象 PJ 絶対パス（`PJ="$(pwd)"`。バッチ経路 #400 本体では queue の project_path に差し替え）。以降のコマンド例に登場する `$PJ` はすべてこの値を指す。

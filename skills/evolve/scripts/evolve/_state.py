...
def persist_last_run_timestamp(
    *, ts: Optional[str] = None, dry_run: bool = False
) -> Dict[str, Any]:
    """apply 境界（`evolve --drain`）で ``last_run_timestamp`` を前進させる（#135/#136）。

    根因（#135）: ``run_evolve(dry_run=False)`` に到達する標準経路が存在せず、
    phases_capture の ``if not dry_run:`` 配下で行う ``last_run_timestamp`` 保存が
    構造的に死蔵していた。結果として fleet queue / ``count_new_sessions`` /
    ``count_new_observations`` / trigger の「前回 evolve 以降」時間フィルタが永久に進まず
    #136 の直接原因になっていた。weak_signals #484 / reward_ema #64 / queue_state #79 と
    同型に apply 境界（drain・tool 文脈・非 dry-run・正準 DATA_DIR）へ移植する。

    drain は observe フェーズを回さないため sessions/observations カウントを持たない。
    それらは書込のみで読み手のいない informational snapshot（``sessions_processed`` /
    ``observations_processed``）なので触らず、時間フィルタが依存する ``last_run_timestamp``
    のみを前進させ、既存 state の他キー（trigger_history 等）は保つ。``dry_run=True`` は
    state に一切触れず件数だけ返す（pitfall_dryrun_stateful_store_write）。

    Returns:
        {"written": int, "last_run_timestamp": str, "dry_run": bool}
    """
    ts = ts or datetime.now(timezone.utc).isoformat()
    if dry_run:
        return {"written": 0, "last_run_timestamp": ts, "dry_run": True}
    state = load_evolve_state()
    state["last_run_timestamp"] = ts

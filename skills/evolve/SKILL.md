...
- **10.1 Reflect**: `reflect_data_count < 0`（degraded sentinel `-1`・#526-3）→ 🟡「Reflect: discover 失敗のため reflect 件数 不明」（数値比較の前に `< 0` を判定する）/ `>= 1` → 🔴 `/evolve-anything:reflect`（未処理 {N}件）/ 0 → 「Reflect: 未処理なし」

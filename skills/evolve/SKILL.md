...
> **ナレーション指示 — Discover / Diagnose 完了後:**
> Discover の発見パターン数（`unmatched_patterns` + `matched_skills` の合計）に応じて
> 以下のいずれか 1 段落を、Step 0.5 でロードした世界観変数を埋め込んで出力すること。
>
> - 3件以上: 「{protagonist_title}は{environment_name}の奥に{issue_name}の気配を感じた。息を潜めながら、それぞれの歪みを確かめていく。」
> - 1〜2件: 「静かな{environment_name}だった。しかし{protagonist_title}の目は一点を捉えた。かすかな{issue_name}の影——見逃せない。」
> - 0件: 「今日の{environment_name}は穏やかだった。{protagonist_title}は静かに巡回を続け、異変なしと確信する。」
...
- `generate_auto_fix_summaries(issues)` で **各 issue を1件ずつ rationale 付きで列挙**する（MUST）。各 entry は `{issue, proposal, rationale}` を持ち、「何を・なぜ・どう直すのか」を issue 単位で提示する
- 列挙後、AskUserQuestion で「一括修正」「個別承認」「スキップ」を選択（MUST）
...
> **ナレーション指示 — Remediation 完了後:**
> 修正を適用したファイル/スキルの数（N件修正 の N）に応じて以下のいずれか 1 段落を出力すること。
>
> - 3件以上: 「{protagonist_title}の手が光る。{environment_name}に{improvement_name}が次々と刻まれた。今夜の仕事は濃かった。」
> - 1〜2件: 「小さな、しかし確かな{improvement_name}が生まれた。{protagonist_title}はそれを手のひらで確かめ、静かに{environment_name}へ戻した。」
> - 0件: 「今夜の{environment_name}は穏やかだった。{protagonist_title}は明日に備えて筆を置く。何も変えないことも、守ることだ。」
...
**提案詳細プロトコルに従う**: アーカイブは元に戻しにくいので、各候補について「スキル名・最終使用日（または "使用記録なし"）・観測期間内の呼び出し回数・作成からの経過」を per-item で提示してから承認を求める。「ゼロ呼び出し」だけでは新規作成直後のスキルを誤ってアーカイブしかねないため、判断材料を出す。
承認されたもののみアーカイブ。
...
> **ナレーション指示 — Prune / Housekeeping 完了後:**
> 以下の 1 文を出力すること。
>
> 「{environment_name}が整えられた。埃が払われ、古い石版が磨かれる。次の冒険のために。」
...
- `status: "insufficient_data"` の場合:
  - 「データ不足: N/30件」と表示
  - `bin/rl-optimize` や `rl-loop` で accept/reject を蓄積する旨を案内
...
>   「✨ {protagonist_title}は新たな段階へ到達した。{旧称号} → **[Lv.{current_level}] {新称号}**」
> - 変化なし（`previous_level` == `current_level`、かつ値あり）:
>   「今日も{environment_name}は健在。**[Lv.{current_level}] {称号}**の境地は揺るがない。」
> - 前回レベル不明（`previous_level` == null / 初回）:
>   「{protagonist_title}の今の境地: **[Lv.{current_level}] {称号}**」
> - env_score が取得できない場合: ナレーションなし（クライマックスをスキップ）。

...
- `status: "insufficient_data"` の場合（**結論を1行で締める** — #400 バグ#5）:
  ...
  - **整合（#479）**: `structural_reason`（`skill_evolve_not_scored`）がある insufficient_data では、
    `next_action`・observability の `calibration_drift` 行（「構造的に対象外」に切替）・Step 2 の
    `has_fitness` 表示の3箇所が「提案が出て初めて母集団が貯まる＝構造的に対象外になり得る」で揃う。
    calibration_drift を「あと N 件で判定可能」と蓄積前提で言い直さない（矛盾するため）。

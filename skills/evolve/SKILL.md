...
**auto_fixable** (confidence ≥ 0.9, impact_scope = file):
- rationale 付きで一括表示し、AskUserQuestion で「一括修正」「スキップ」を選択（MUST）
- 承認後: `fix_stale_references()` 等を実行 → `verify_fix()` + `check_regression()` で2段階検証
- regression 検出時: `rollback_fix()` で復元し manual_required に格上げ
- 結果を `record_outcome()` で記録

**proposable** (confidence ≥ 0.5, or impact_scope = project):

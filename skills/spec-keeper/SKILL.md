...
5. **用語集 drift チェック（`CONTEXT.md` が存在する場合のみ）**:
   ```bash
   [ -f CONTEXT.md ] && python3 scripts/lib/glossary_drift.py CONTEXT.md SPEC.md CLAUDE.md || true
   ```
   - **構造 drift（exit 1: スキーマ不一致 / 重複定義 / 初出欠落）** → CONTEXT.md を Edit で修正してから完了する（MUST）。腐った用語集は無いより悪い
   - **advisory（未登録 jargon 候補）** → 一覧をユーザーに提示。今回の変更で新概念が増えていれば CONTEXT.md への 1 行追記を提案（決定論なので誤検出を含む。捏造せず、意味が確認できる用語のみ追記）

更新は最小限に — 変更があった箇所だけ編集し、全面書き換えはしない。

6. **README.md 更新（README.md が存在する場合のみ）**:
...
| 用語集（CONTEXT.md） | `python3 scripts/lib/glossary_drift.py CONTEXT.md SPEC.md CLAUDE.md` | 構造 drift は exit 1。advisory は未登録 jargon |

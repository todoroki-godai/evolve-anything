...
4. 一時ディレクトリ配下の削除（default: `/tmp/evolve-anything-*`。CRITICAL: `claude-` / `gstack-` はランタイム領域と衝突するためデフォルト対象外。`CLAUDE_PLUGIN_OPTION_cleanup_tmp_prefixes` / manifest userConfig でカンマ区切り拡張可能、scanner 側の安全ネット (`claude-<uid>` / `claude-mcp-*` 常時除外) で防御維持）
5. マージ済みブランチ名から推定した関連 Issue の close 候補提案（**自動 close はしない**）
6. 元 PR の Test plan 未完了チェックの残件リマインド
7. Claude Code プロジェクト状態のパージ（会話履歴・タスク・ファイル変更履歴の完全削除、**オプション・不可逆**）
...
デフォルト prefix は **`evolve-anything-` のみ**。`claude-` / `gstack-` は Claude Code ランタイム (`/tmp/claude-<uid>`) や MCP bridge (`/tmp/claude-mcp-*`)、gstack 作業ディレクトリ (`/tmp/gstack-work`) と衝突し、削除するとセッションが壊れる危険があるため**デフォルトから除外**している。prefix 拡張は `manifest.userConfig` の `cleanup_tmp_prefixes`（またはその環境変数 `CLAUDE_PLUGIN_OPTION_cleanup_tmp_prefixes`）でカンマ区切りで追加可能（例: `"evolve-anything-,claude-sandbox-,gstack-scratch-"`）。scanner 側 `_DEFAULT_TMP_EXCLUDE_PATTERNS` で `claude-<uid>` / `claude-mcp-*` は二重に保護しているため、ユーザーが `claude-` prefix を追加してもランタイム領域は守られる。

各ディレクトリについて個別に:

```
一時ディレクトリ `/tmp/evolve-anything-bench-abc` を削除しますか？
(rm -rf /tmp/evolve-anything-bench-abc)
...
`rm -rf` 実行前に、パスが `/tmp/evolve-anything-` で始まることを再確認する（防衛）。
...
#### カテゴリ 7: Claude Code プロジェクト状態のパージ（オプション）

**デフォルトはスキップ。不可逆**: プロジェクトの会話履歴・タスク・ファイル変更履歴が完全に削除される。
ユーザーが「CC の履歴を消したい」「プロジェクトをリセットしたい」と明示した場合のみ実行する。

まず dry-run で影響範囲を確認する:

```bash
claude project purge --dry-run
```

確認後、`AskUserQuestion` で承認を取る:

```
Claude Code プロジェクト状態（会話履歴・タスク・ファイル変更履歴）を完全削除しますか？
この操作は不可逆です。
...
承認時: `claude project purge -y` を実行。
...
| **`/evolve-anything:cleanup`** | **マージ・デプロイ後の痕跡掃除** |
